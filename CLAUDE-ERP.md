...
- The remaining ~60 `<page>-main { overflow-y: auto }` rules are **dead weight, not bugs**. They're preserved for now to keep this commit small. A future Phase UX-Scroll-Followup can sweep them for hygiene; risk of regression is zero (the rules don't trigger).
...
**Healthcheck extended** with 5 new assertions:
...
`node backend/scripts/healthcheckSalesDiscount.js` PASSES **40/40** (was 33/33 pre-hardening). Vite green in 13.39s. Backend syntax clean on touched files.

**Live Playwright UI smoke (deferred from original ship — closed Apr 30 2026)**: logged in as president (yourpartner@viosintegrated.net), navigated to `/erp/sales/entry`, typed qty=10 / unit_price=100 / discount=10 in the new fields → per-line Total cell renders **900.00** and the row-header total renders **₱900.00**, exactly matching the BIR net method `1000 - 10% = 900`. Screenshot at [r2-discount-math-smoke.png](r2-discount-math-smoke.png). Confirms `computeLineTotal` client math + 9-column desktop grid + Disc % spinbutton wiring (with the BIR-trade-discount tooltip) are all live.
