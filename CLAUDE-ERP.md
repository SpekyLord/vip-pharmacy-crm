...
> **Version**: 9.4
> **Status**: Phase **32R-Sales-Validate-Proxy-Scope (May 8 2026, ✅ shipped — pending UI smoke ratification on dev)** — closes the second half of the Phase 32R-Transfer-Stock-Scope (May 7) AND-mode regression in `validateSales`. The May-7 same-day patch (Phase 32R-Validate-Privileged-Snapshot) dropped `bdm_id` to `null` for privileged callers — that fixed the president/admin/finance path but left **two coupled holes**: (A) **Proxy entries** (any sale where `recorded_on_behalf_of` is set, e.g. row 4806 "Cecilia Olarte Paderes" today) — non-privileged BDMs still hit `req.bdmId` (the proxy/data-entry user) intersected with `row.warehouse_id`, which under AND mode collapses to "available 0, requested N" even when the row OWNER has stock at the warehouse. (B) **Validate-pass / Submit-fail risk** — submit-side ([salesController.js:1146](backend/erp/controllers/salesController.js#L1146)) uses `row.bdm_id` (owner), validate-side dropped to `null` (entity-wide-at-warehouse). Privileged user could pass Validate against another BDM's stock at a shared warehouse, then surface INSUFFICIENT_STOCK at Submit with no advance warning. **Fix** ([salesController.js:731-797](backend/erp/controllers/salesController.js#L731-L797), [:961-1010](backend/erp/controllers/salesController.js#L961-L1010)): per-`(effectiveBdmId, warehouseId)` snapshot cache + per-group deduction tracker, keyed off ROW attributes. `effectiveBdmId = (privileged && !row.warehouse_id) ? null : row.bdm_id` mirrors submit's contract verbatim. Per-group deducted Map prevents one row's allocation from bleeding into a different (BDM, warehouse) pool — correctly reflects that different owners at different warehouses read separate InventoryLedger rows. Insufficient-stock error now appends `(scope: warehouse X ∩ BDM Y)` so admins can self-diagnose data-quality cases (e.g. row's warehouse_id stale relative to where stock actually lives). **Subscription-readiness preserved**: zero new lookup categories. Privileged-role set still flows from `req.isPresident` / `req.isAdmin` / `req.isFinance` (lookup-driven via PROXY_ENTRY_ROLES + role baselines). **Cascading downstream sanity verified**: (1) submit-side `consumeFIFO` / `consumeSpecificBatch` untouched — `row.bdm_id` semantics preserved. (2) `creditNoteController.buildStockSnapshot` import unchanged. (3) `OPENING_AR` / `SERVICE_INVOICE` skip-inventory branch preserved. (4) BDM (non-proxy, non-privileged) self-validate — `row.bdm_id === req.bdmId === own _id`, no behavior change vs Phase 32R-Validate-Privileged-Snapshot. (5) Multi-row Validate batch (privileged user validating mixed proxy + self entries across BDMs/warehouses) — each row scopes to its own pool, no cross-pool snapshot leakage. **Healthcheck**: [healthcheckSalesValidateProxyScope.js](backend/scripts/healthcheckSalesValidateProxyScope.js) NEW. **Prod-data follow-up**: row 4806 specifically — if Validate continues to error after deploy, the row's `warehouse_id` likely points to a warehouse where the owner has 0 Philvan|1g stock; the new scope-suffix in the error message will name the warehouse + BDM so admin can decide whether to (a) edit the row's warehouse_id, (b) IST stock to that warehouse, or (c) reassign the sale to a different BDM via the Proxy entry flow. erp-remote no-push policy in force.

> **Previously**: Phase **32R-Validate-Privileged-Snapshot (May 7 2026, ✅ shipped — pending UI smoke ratification on dev)** — fixes "Cannot validate sales transaction — Insufficient stock for product Philvan|1 g: available 0, requested 18" reported with screenshot when president clicked Validate on `/erp/sales`. Root cause is a sister-regression to Phase 32R-Transfer-Stock-Scope: [tenantFilter.js:28](backend/erp/middleware/tenantFilter.js#L28) stamps `req.bdmId = req.user._id` on every authenticated user (including president); [salesController.validateSales:737](backend/erp/controllers/salesController.js#L737) then passed that president-`_id` into `buildStockSnapshot` whenever the row had a `warehouse_id`. Pre-32R, `buildStockMatch`'s XOR semantics let `warehouse_id` win over `bdm_id` so the call accidentally returned the right warehouse-scoped slice. Phase 32R switched to AND, which intersected the president's own user._id (who owns zero InventoryLedger rows) with the warehouse → empty snapshot → every line item flagged Insufficient Stock. Submit-time consume (lines 168 + 1120) reads `row.bdm_id` (the row owner's BDM, not the calling user) so submit was never affected — only validate had the regression. **Fix**: unconditionally drop `bdm_id` scoping for privileged callers (president/admin/finance) at the validate snapshot — warehouse_id alone is the correct scope, and warehouse-less validate calls become entity-wide which is the documented behavior. Non-privileged BDMs continue to see only their own stock (`req.bdmId = own _id`). **Subscription-readiness preserved**: zero new lookup categories — re-uses `req.isPresident` / `req.isAdmin` / `req.isFinance` flags already set by the existing tenantFilter middleware. Future role additions (e.g. CFO, Controller) inherit the same gate by being added to `isPresidentLike` in [constants/roles.js](backend/constants/roles.js). **Cascading downstream sanity verified**: (1) `consumeFIFO` / `consumeSpecificBatch` (submit) — untouched; row-owner `bdm_id` semantics preserved (lines 168, 1120). (2) `creditNoteController` imports `buildStockSnapshot` but never calls it — no behavioral change. (3) FIFO engine `buildStockMatch` AND-mode (Phase 32R-Transfer-Stock-Scope) preserved — the fix runs ABOVE that helper. (4) BDM (contractor) validate path unchanged — still scopes by `req.bdmId` = own `_id`. **Targeted assertion**: 8/8 PASS (privileged-snapshot fix, submit untouched, fifoEngine intersection unchanged). **Phase 32R-Transfer-Stock-Scope healthcheck**: 40/40 PASS unchanged. **Vite build**: ✓ green. **Live API + Playwright UI smoke deferred** — backend dev process was stopped between fix and smoke; the user can restart and validate any sale row as president to confirm `Insufficient Stock` is gone. **Files modified (1)**: [backend/erp/controllers/salesController.js](backend/erp/controllers/salesController.js#L731-L756) (privileged-bdmId-drop + Phase tag comment). Plus 2 docs: this CLAUDE-ERP.md status banner refresh to v9.3 + [docs/PHASETASK-ERP.md](docs/PHASETASK-ERP.md) new Phase 32R-Validate-Privileged-Snapshot section. erp-remote no-push policy in force.

> **Previously**: Phase **P1.2 Slice 6.2 follow-up (Phase 2.1) — GrnEntry picker sub_type narrowing + WAYBILL upload picker (May 7 2026, ✅ shipped — pending UI smoke ratification on dev)** — closes the two follow-up bullets that Slice 6.2 deferred. The OCR picker next to "Scan Undertaking Paper" on `/erp/grn` now narrows the GRN feed via `subTypeFilter={{ GRN: 'BATCH_PHOTO' }}` (UNCATEGORIZED stays unfiltered as Quick Capture fallback) so a courier waybill capture cannot accidentally land in the OCR scan modal. The waybill upload card gains a second `<PendingCapturesPicker>` filtered to GRN/WAYBILL — proxy taps "From BDM Captures", picks one, picker pulls the signed S3 URL → Blob → File via fetch-mode (NOT skipFetch — no server-side capture-id endpoint for waybill exists), routes through the existing `handleWaybillUpload` → `processDocument(file, 'WAYBILL')` skip-OCR pipeline, and `waybill_photo_url` lands the same way gallery upload does. **Two pickers, no cross-bleed.** **Subscription-readiness preserved**: zero new lookup categories — `subTypeFilter` is already a runtime prop. Sub-permission gates (`CAPTURE_LIFECYCLE_ROLES.PROXY_PULL_CAPTURE`), `entity_id` (Rule #19), `bdm_id` (Rule #21) posture all unchanged. **Cascading downstream sanity verified**: `/erp/grn` save flow unchanged (`waybill_photo_url` semantic identical, `pendingCaptureId` flow on submit unchanged), Undertaking mirror (`createGrn` → `inventoryController` writes both copies) unchanged, `linkCaptureToDocument` does NOT fire on waybill picker pick (correct — REVIEW_WORKFLOWS does not include GRN; the WAYBILL CaptureSubmission stays PENDING_PROXY until proxy explicitly marks complete via Capture Archive override path), Slice 9 `paper_received` attestation flow on the WAYBILL CaptureSubmission row unchanged. **Healthcheck**: [backend/scripts/healthcheckCaptureHub.js](backend/scripts/healthcheckCaptureHub.js) gains 6 new asserts under "Phase P1.2 Slice 6.2 follow-up (Phase 2.1)" section — **666/666 PASS** (was 660/660). **Files modified (3)**: [frontend/src/erp/pages/GrnEntry.jsx](frontend/src/erp/pages/GrnEntry.jsx) (subTypeFilter on existing OCR picker; new second picker mount in waybill panel), [frontend/src/erp/components/WorkflowGuide.jsx](frontend/src/erp/components/WorkflowGuide.jsx) (`grn-entry` tip extended with Phase 2.1 follow-up paragraph — Rule #1), [backend/scripts/healthcheckCaptureHub.js](backend/scripts/healthcheckCaptureHub.js) (new section, 6 asserts). Plus 2 docs: this CLAUDE-ERP.md status banner refresh to v9.2 + [docs/PHASETASK-ERP.md](docs/PHASETASK-ERP.md) new Phase 2.1 follow-up section between Slice 6.2 and Slice 6.1. Subscriber-overrideable D/M flag per sub_type still deferred — no subscriber needs it today; backend `attach-existing-capture` endpoint for waybill considered and rejected (same-origin in production sidesteps the CORS lurking-bug). erp-remote no-push policy in force.

> **Previously**: Phase **32R-Transfer-Stock-Scope (May 7 2026, ✅ shipped + UI+API ratified on dev — uncommitted)** — fixes "Internal Stock Reassignment product dropdown is empty even after picking Source Custodian + Source Warehouse" reported with screenshot. Two coupled bugs converged: **(A) `fifoEngine.buildStockMatch` was XOR**, not AND — when both `bdmId` and `opts.warehouseId` were provided, the warehouse_id branch fired in an `else if` and the bdm_id filter was silently dropped. For shared warehouses (e.g. `BAC — VIP Bacolod`, `ACC — Shared Services`) holding stock from multiple BDMs, the dropdown either returned the union (wrong product visible to wrong BDM) or the intersection collapsed to zero matches in the entity scope the caller actually owned. **(B) Frontend never forwarded `warehouse_id` to the dropdown queries** — `TransferOrders.jsx` called `getMyStock(newBdmId)` (Internal) and `getMyStock(newBdmId, form.source_entity_id)` (IC) without warehouse_id and never re-fetched on warehouse change. The Internal modal also dropped `entity_id`, falling through to `req.entityId` (top-right working-entity header), which can differ from the BDM list's home entity for privileged callers — that mismatch hid Patrocinio's stock from Jay Ann's view in the screenshot. **Fix**: (1) [backend/erp/services/fifoEngine.js](backend/erp/services/fifoEngine.js) `buildStockMatch` switched from XOR to AND — both filters now intersect when both are provided; backwards-compatible for callers passing only one. AND mode is also a strict-superset safety improvement on consume-side callers (salesController, consignmentController, interCompanyService) that already pass both: each InventoryLedger row carries a single (bdm_id, warehouse_id) pair, so the intersection never excludes legitimate rows. (2) [frontend/src/erp/pages/TransferOrders.jsx](frontend/src/erp/pages/TransferOrders.jsx) — IC and Internal modals each get a useEffect keyed on `(source_bdm_id, source_warehouse_id, source_entity_id)` that re-fetches stock + clears the batch cache on change. The Source Custodian onChange handlers are now pure state setters (the useEffect handles refetch, no race). `getBatches` calls forward `source_warehouse_id`. The Internal modal's `cacheKey` is extended to include `source_warehouse_id` so two warehouses' batches stay separate. The Product `<SelectField>` is `disabled={!sourceReady}` with placeholder `"Pick custodian + warehouse"` until both are picked; an amber hint banner explains why. (3) [frontend/src/erp/components/WorkflowGuide.jsx](frontend/src/erp/components/WorkflowGuide.jsx) `transfers` step 4 + tip explain the new BDM ∩ warehouse intersection contract (Rule #1). **Subscription-readiness preserved**: zero new lookup categories. The fix preserves every existing role gate, period lock, two-person approval rule, and the Phase G4.5dd-r2 same-custodian rebalance flow. `req.bdmId` (non-privileged users) and `req.entityId` (Rule #19) posture unchanged. `?bdm_id=` and `?entity_id=` overrides for privileged users (Rule #21) preserved. **Cascading downstream sanity verified**: (1) salesController.consumeFIFO/consumeSpecificBatch — passes `(entity, submitBdmId, ..., {warehouseId})`; pre-fix XOR consumed from "warehouse Y, any BDM"; post-fix AND consumes from "warehouse Y AND that BDM" — same rows in single-owner warehouses, fewer rows (correctly) in shared warehouses. (2) consignmentController same pattern. (3) interCompanyController same pattern. (4) creditNoteController.buildStockSnapshot — passes only bdmId, opts (no productId); behavior identical. (5) inventoryController.getMyStock display + getBatches display — privileged users see the correct (bdm,warehouse) intersection now; admin querying their own bdm_id at a shared warehouse sees only their slice (this was the original symptom). **Healthcheck**: [backend/scripts/healthcheckTransferStockScope.js](backend/scripts/healthcheckTransferStockScope.js) NEW — **40/40 PASS** asserts the AND mode + endpoint contracts + frontend hook signatures + IC + Internal modal wiring + banner copy + consume-side callers unchanged. **Regression**: `healthcheckCaptureHub.js` **660/660 PASS** (no regression on Phase P1.1 + P1.2 Slices 1-9). **Vite build**: ✓ green in 43.92s. **Live API smoke** (president on dev VIP entity): warehouse `BAC — VIP Bacolod` (`69d1098d8a949881a5eac252`) holds stock from 2 BDMs (BDM A: 30100u/2 products, BDM B: 7415u/19 products). Pre-fix `?bdm_id=A&warehouse_id=BAC` would have returned 37515u/21 products (warehouse-only). Post-fix returns **30100u/2 products** for BDM A's slice; **7415u/19** for BDM B's slice; **37515u/21** for warehouse-only (no bdm); **40897u/4** for BDM A across all warehouses. **Live UI smoke** (Playwright as president on dev): `/erp/transfers` → Internal tab → "+ Reassign Stock" → modal opens cleanly, hint banner shows, Product dropdown placeholder reads "Pick custodian + warehouse"; pick Source Custodian = Gregg Louie Vios + Source Warehouse = BAC → Product dropdown enables and lists exactly the 2 products (Anaway Forte 30000 TABLET + Ascorsaph-Zee 100 CAPSULE) from this BDM at this warehouse, totals match API exactly; switch warehouse to ILO-MAIN → dropdown re-fetches and shows 2 different products (Viprazole 4998 VIAL + Viptriaxone 5799 VIAL = 10797 units, again matching API). IC modal also gated correctly — banner present, placeholder + disabled state confirmed. 0 console errors throughout. **Files modified (4)**: [backend/erp/services/fifoEngine.js](backend/erp/services/fifoEngine.js), [frontend/src/erp/pages/TransferOrders.jsx](frontend/src/erp/pages/TransferOrders.jsx), [frontend/src/erp/components/WorkflowGuide.jsx](frontend/src/erp/components/WorkflowGuide.jsx), [backend/scripts/healthcheckTransferStockScope.js](backend/scripts/healthcheckTransferStockScope.js) (NEW). Plus 2 docs: this CLAUDE-ERP.md status banner refresh to v9.1 + [docs/PHASETASK-ERP.md](docs/PHASETASK-ERP.md) new Phase 32R-Transfer-Stock-Scope section. erp-remote no-push policy in force.

> **Previously**: Phase **32R-GRN-Approve-UX (May 7 2026, ✅ shipped + UI+API ratified on dev — uncommitted)** — fixes "Can't approve VIP GRN ENTRY" reported with screenshot. Two coupled bugs: **(A) ReferenceError in `approveGrnCore`** — `getGrnSetting` was destructured from `require('../services/undertakingService')` inside `createGrn`'s function scope (line 566) but referenced unqualified inside `approveGrnCore` (line 869, called by both the direct GRN approve HTTP handler AND `undertakingController.postSingleUndertaking`'s auto-approve cascade). Every direct GRN approval threw `ReferenceError: getGrnSetting is not defined` and HTTP 500'd. Fix: hoist the require to module scope ([backend/erp/controllers/inventoryController.js](backend/erp/controllers/inventoryController.js#L21-L26)) — single line, both call sites now resolve. **(B) Frontend silent-error swallow** — [GrnEntry.handleApprove](frontend/src/erp/pages/GrnEntry.jsx) caught the 500 (and every other non-`approval_pending` error) into `console.error` only. No toast, no modal — the user clicked Approve, nothing visible happened, and they reasonably concluded the button was broken. Fix: replace `console.error` with `showSuccess`/`showError` from errorToast.js (same react-hot-toast pattern every other ERP page uses); when the backend ships `data.undertaking_id` in a 400 (Phase 32R UT-not-acknowledged guard), navigate the user straight to that UT page with one click. **(C) Backend list enrichment** — `getGrnList` now joins each GRN with its linked Undertaking summary (`{_id, undertaking_number, status, acknowledged_at}`, skipping reversed UTs via the partial unique index). One `$in` query per page, no N+1. **(D) Frontend gate UX** — new `grnApproveState(g)` helper disables the Approve button when the linked UT is not ACKNOWLEDGED, surfaces the reason via `title=` tooltip, and renders an "Open Undertaking →" deep-link CTA underneath. President bypass mirrors the controller (`req.isPresident` short-circuit). The Undertaking column now shows `UT-XXX → STATUS` colored by ACK/REJECTED/other. Reject button stays available regardless of UT state. **(E) Banner refresh** — [WorkflowGuide grn-entry](frontend/src/erp/components/WorkflowGuide.jsx) step 6 added (Rule #1 — banners must reflect current behavior). **Subscription-readiness preserved**: zero new lookup categories. The fix is purely UX-routing for the existing `MODULE_DEFAULT_ROLES.INVENTORY` (Phase G4) gate + the existing president-bypass invariant. **Cascading downstream sanity verified**: (1) `undertakingController.postSingleUndertaking` cascade — also fixed by the require-hoist; the auto-approve path was hitting the same ReferenceError. (2) `interCompanyController` GRN approvals — uses `approveGrnCore`, also fixed. (3) `getGrnList` consumers — only `frontend/src/erp/hooks/useGrn.js` calls it, and the `undertaking` field is purely additive (existing keys unchanged). (4) Period-lock + waybill + authority-matrix gates downstream of `approveGrnCore` are unchanged. **Healthcheck**: [backend/scripts/healthcheckGrnApproveUx.js](backend/scripts/healthcheckGrnApproveUx.js) NEW — **30/30 PASS** asserts the require hoist + listing enrichment + frontend handler + button gate + banner. **Regression healthcheck**: `healthcheckCaptureHub.js` **660/660 PASS** (no regression on Phase P1.1 + P1.2 Slices 1-9). `healthcheckInternalTransferProxy.js` 33/33, `healthcheckProductGlobalization.js` 56/56. **Vite build**: ✓ green in 16.71s. **Live API smoke** (president on dev VIP entity): seeded 3 PENDING GRNs (no UT / DRAFT UT / ACKNOWLEDGED UT) — all three approved via `POST /api/erp/inventory/grn/:id/approve` returned **HTTP 200** + `"GRN approved — stock updated"`. Pre-fix the same call returned 500 + `getGrnSetting is not defined`. **Live UI smoke**: Playwright walked `/erp/grn` → PENDING tab → all 3 smoke rows rendered with the new "UT-XXX → STATUS" column + correct status colors (ACKNOWLEDGED green, DRAFT amber, none gray "View →"); president-bypass left all Approve buttons enabled with `title="President bypass — direct GRN approve allowed"`; click triggered the actual approval, server returned 200, success toast fired. Pre-fix, the same click silently console.error'd. Smoke fixtures + ledger residue cleaned up after ratification. **Files modified (4)**: [backend/erp/controllers/inventoryController.js](backend/erp/controllers/inventoryController.js) (hoist + getGrnList enrichment), [frontend/src/erp/pages/GrnEntry.jsx](frontend/src/erp/pages/GrnEntry.jsx) (handleApprove + grnApproveState helper + table gate + card gate + Undertaking column), [frontend/src/erp/components/WorkflowGuide.jsx](frontend/src/erp/components/WorkflowGuide.jsx) (grn-entry step 6), [backend/scripts/healthcheckGrnApproveUx.js](backend/scripts/healthcheckGrnApproveUx.js) (NEW). Plus 2 docs: this CLAUDE-ERP.md status banner refresh to v9.0 + [docs/PHASETASK-ERP.md](docs/PHASETASK-ERP.md) new Phase 32R-GRN-Approve-UX section. erp-remote no-push policy still in force; commit recipe in handoff. **Why bug A was hidden so long**: the auto-approve cascade through Undertaking ACK is the dominant production path (Phase 32R intentionally routes approvals through UT, not directly through GRN), and that path's `approveGrnCore` invocation also hit the ReferenceError but the `withTransaction` wrapper rolled it back as a generic 500. Direct-GRN approves are rare in normal use (UT-ACK is the dominant path); when one user finally tried (after a UT they wanted to skip), the silent frontend swallow masked the underlying ReferenceError. Today's user report finally surfaced it.

> **Previously**: Phase **P1.2 Phase 1 — Capture cleanup + Option D BIR audit gate (May 6 2026, ✅ shipped — pending UI smoke ratification on dev)**: Three coupled changes that close an audit-credibility gap exposed by Round 2C and clean up two unused enum slots. (1) `workflow_type='CWT_INBOUND'` is dropped — CWT now lives as a sub_type of `COLLECTION` (hospitals send CR + DEPOSIT + CWT together as one collection package). (2) `workflow_type='PETTY_CASH'` is dropped — tile was never shipped on the Capture Hub; enum slot was speculative. (3) **Two-step BIR receive on `/erp/bir/2307-IN` (Option D audit gate)** — photo evidence (`cert_2307_url`) attaches but does NOT unlock the 1702 credit; finance must ALSO tick a "paper-in-Iloilo" checkbox that flips `status='RECEIVED'` + stamps `CwtLedger.physical_received_at`. The gap state (cert URL non-empty + status=PENDING_2307 + physical_received_at NULL) is the new **PHOTO_ATTACHED** pseudo-status, surfaced as a tab + amber pill on the inbound page. **Why the gate matters:** BIR RR No. 2-98 + 1604-E line-item docs require the original paper certificate as documentary evidence for the 1702 "Less: Creditable Tax Withheld" credit. Round 2C let finance pick a BDM photo and immediately mark RECEIVED → 1702 credit unlocked, which under audit could be disallowed → 25% surcharge + 12% interest on the underpaid amount. **Migration** ([backend/scripts/migrateCwtInboundToCollectionSubtype.js](backend/scripts/migrateCwtInboundToCollectionSubtype.js)) — dry-run-by-default, idempotent, raw-collection access (works regardless of model enum state). 2 CWT_INBOUND rows flipped on dev cluster, 0 PETTY_CASH. Migration MUST run before model enum is narrowed (otherwise existing rows fail Mongoose validation on next save). **Frontend wiring**: [Bir2307InboundPage.jsx](frontend/src/erp/pages/Bir2307InboundPage.jsx) gains `paperReceived` state (default UNCHECKED on fresh open), the new "Photo attached" tab between Pending and Received, the paper-received checkbox in the Mark-Received modal (disabled until cert URL non-empty, cites BIR RR No. 2-98 in the helper text), Save button label branches on `paperReceived` (`'Save — Mark RECEIVED'` checked vs `'Save Photo Only'` unchecked), and `STATUS_META.PHOTO_ATTACHED` row pill (amber `#fef3c7`/`#92400e`). Picker workflowTypes flip from `['CWT_INBOUND','UNCATEGORIZED']` → `['COLLECTION','UNCATEGORIZED']` + new `subTypeFilter={{COLLECTION:'CWT'}}` prop on [PendingCapturesPicker.jsx](frontend/src/erp/components/PendingCapturesPicker.jsx). [BdmCaptureHub.jsx](frontend/src/erp/pages/mobile/BdmCaptureHub.jsx) CWT tile flips from `key:'CWT_INBOUND'` to `key:'COLLECTION', sub_type:'CWT'`. [BdmReviewQueue.jsx](frontend/src/erp/pages/mobile/BdmReviewQueue.jsx) gains `COLLECTION_CWT` composite-key entry in WORKFLOW_ICONS / WORKFLOW_COLORS / WORKFLOW_LABELS (legacy `CWT_INBOUND` + `PETTY_CASH` kept as defensive fallbacks). [ProxyQueue.jsx](frontend/src/erp/pages/proxy/ProxyQueue.jsx) WORKFLOW_OPTIONS dropdown drops PETTY_CASH and adds COLLECTION + UNCATEGORIZED (Quick Capture catchall). **Backend wiring**: [CwtLedger.js](backend/erp/models/CwtLedger.js) gains `physical_received_at: { type: Date, default: null, index: true }` — distinct from existing `received_at` (the digital-attach timestamp). Indexed because the new PHOTO_ATTACHED tab filters rows where `cert_2307_url IS NOT NULL && physical_received_at IS NULL`. [cwt2307ReconciliationService.markReceived](backend/erp/services/cwt2307ReconciliationService.js) signature gains `paper_received` (default `true` for backward compat with non-Phase-1 callers; Phase-1 frontend always passes explicitly); cert_* fields always stamped, paper attestation only when `paper_received===true`; rejects `paper_received=true` with empty `cert_2307_url` (defense-in-depth mirror of disabled-checkbox UI gate); photo-only saves on already-RECEIVED rows preserve the prior paper attestation (don't demote). `markPending` clears `physical_received_at` along with `received_at`/`received_by`. `listInboundRows` surfaces `physical_received_at` so the frontend can render PHOTO_ATTACHED. [birController.markReceived2307Inbound](backend/erp/controllers/birController.js) extracts `paper_received` from body, coerces with backward-compat default `true` if absent, forwards to service. Audit log line `[BIR_2307_INBOUND_MARK_RECEIVED]` extended with `paper_received` + `final_status` + `physical_received_at` so admin can grep historical events to see which receives unlocked the 1702 credit vs which only attached photo evidence. [captureSubmissionController.js](backend/erp/controllers/captureSubmissionController.js) `VALID_TYPES` drops `CWT_INBOUND` + `PETTY_CASH`; `VALID_SUB_TYPES_BY_WORKFLOW.COLLECTION` adds `'CWT'` (now 4 values: CR / DEPOSIT / PAID_CSI / CWT); `REVIEW_WORKFLOWS` (in both `completeCapture` AND `linkCaptureToDocument`) drops `CWT_INBOUND` (COLLECTION already covers it via sub_type); `getProxyQueue` accepts new optional `sub_type` query param + threads to the Mongo filter; `DIGITAL_ONLY` arrow does NOT classify COLLECTION+CWT as digital-only — paper certificate must arrive at the Iloilo office, so `physical_required=true` is correct. **Cascading downstream sanity verified**: Phase J7 1702 aggregator reads `status='RECEIVED'` so it correctly excludes PHOTO_ATTACHED rows from the credit (their `status` is still `'PENDING_2307'`); pending-exposure computation correctly counts photo-attached rows as exposure (they ARE still credit-at-risk until paper arrives); J6 inbound posture card splits by `status` so PHOTO_ATTACHED rows count under PENDING_2307; Capture Archive (Slice 8) folder tree groups by `(workflow_type, sub_type)` so post-migration CWT captures appear under `COLLECTION / CWT` (vs the legacy `CWT_INBOUND` standalone leaf); Slice 9 paper-received attestation gate lives on `CaptureSubmission.physical_received_at` (capture-side), distinct from `CwtLedger.physical_received_at` (CWT-ledger-side) — the two paper-received fingerprints serve different surfaces and don't interact; Round 2C linkCaptureToDocument still fires when `capture_id` is forwarded, regardless of `paper_received` value (the capture lifecycle advance is independent of the BIR audit gate). **Subscription-readiness preserved**: zero new lookup categories. Paper-attestation gate is hardcoded by BIR-regulation requirement, not subscriber-tunable. Picker reuses `CAPTURE_LIFECYCLE_ROLES.PROXY_PULL_CAPTURE` (defaults `[admin, finance, president]`) + `BIR_ROLES.RECONCILE_INBOUND_2307` (defaults `[admin, finance, bookkeeper]`); subscribers retune via Control Center → Lookup Tables (Rule #3 / Rule #19). `entity_id` (Rule #19) and `bdm_id` (Rule #21) posture unchanged on every endpoint touched.
...
>
> **CRM-side phase note (May 06, 2026 — Phase A.5.3 + A.5.6)**: Closes the dedup story. Two `VIP_CLIENT_LIFECYCLE_ROLES` codes flip from forward-compat to consumer-active: `JOIN_COVERAGE_AUTO` (admin/president defaults — adds the calling user to `Doctor.assignedTo[]` via `$addToSet`; subscribers loosen to include `staff` for instant BDM self-join) and `JOIN_COVERAGE_APPROVAL` (admin/president defaults — posts a MessageInbox approval request to admins; subscribers loosen to include `staff` so BDMs can request without admin password-share). Wired by new endpoint `POST /api/doctors/:id/join-coverage` ([doctorRoutes.js](backend/routes/doctorRoutes.js)). The seed rows in [lookupGenericController.js SEED_DEFAULTS](backend/erp/controllers/lookupGenericController.js) already had `insert_only_metadata: true`, so admin tweaks survive future re-seeds (Rule #19 hot-config posture). **A.5.6 ERP impact**: zero — the `mergedInto` resolver in [visitController.createVisit](backend/controllers/visitController.js) is CRM-side only (offline-replayed Visit pointing at a now-merged loser auto-redirects to the winner). It does NOT change any ERP-side cascade, ERP-side rebate-accrual, or ERP-side audit posture. The merge cascade (Phase A.5.5) already covers all 6 ERP-side FK paths; this resolver only affects the CRM Visit collection. **Healthcheck**: 80/80 PASS in [backend/scripts/healthcheckPhaseA5_3_6.js](backend/scripts/healthcheckPhaseA5_3_6.js).
>
