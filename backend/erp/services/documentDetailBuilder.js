...
function buildIncomeDetails(item) {
  ...
  return {
    ...
    source_refs: {
      smer_id: sr.smer_id || null,
      collection_count: (sr.collection_ids || []).length,
      expense_count: (sr.expense_ids || []).length,
      pnl_report_id: sr.pnl_report_id || null,
    },
...
function buildSalesDetails(item) {
  ...
  return {
    ...
    reopen_count: item.reopen_count || 0,
    created_at: item.created_at || null,
    petty_cash_fund_id: item.petty_cash_fund_id || null,
    ...
    csi_received_at: item.csi_received_at || null,
    dunning_ready: !!item.csi_received_photo_url,
...
function buildExpensesDetails(item) {
  ...
  const lineCount = item.line_count || allLines.length;
  const LINES_SHOWN = 10;
  const calfLines = allLines.filter(l => l.calf_id).length;
  return {
    period: item.period,
    cycle: item.cycle,
    status: item.status,
    ...
    lines_shown: Math.min(LINES_SHOWN, allLines.length),
    lines_truncated: Math.max(0, allLines.length - LINES_SHOWN),
    lines_with_calf_count: calfLines,
    lines: allLines.slice(0, LINES_SHOWN).map(l => ({
...
function buildPrfCalfDetails(item) {
  return {
    ...
    linked_expense_id: item.linked_expense_id || null,
    linked_expense_line_count: (item.linked_expense_line_ids || []).length,
    funding_card_id: item.funding_card_id || null,
    petty_cash_fund_id: item.petty_cash_fund_id || null,
...
function buildIcTransferDetails(item) {
  ...
  return {
    ...
    target_event_id: item.target_event_id || null,
    created_at: item.created_at || null,
...
function buildPettyCashDetails(item) {
  return {
    ...
    remittance_id: item.remittance_id || null,
    created_at: item.created_at || null,
