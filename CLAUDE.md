...
11. **Phase A.5 canonical VIP Client key** (Apr 2026, A.5.1 + A.5.5 + A.5.2-prep shipped; A.5.2 flip pending admin dedup): `Doctor.vip_client_name_clean` is auto-maintained by pre-save + pre-findOneAndUpdate hooks as `lastname|firstname` (lowercased). Mirrors `Customer.customer_name_clean` (Phase G5) / `Hospital.hospital_name_clean`. **A.5.2-prep shipped May 04 2026**: (a) backfilled `vip_client_name_clean` + `primaryAssignee` on all 734 active doctors via `node backend/scripts/migrateVipClientCanonical.js --apply` so the merge tool's `findCandidates` can surface duplicates (it filters on `vip_client_name_clean: { $exists: true, $ne: null, $ne: '' }`); (b) flipped the migration script's target index shape from plain `{ unique: true }` to `{ unique: true, partialFilterExpression: { mergedInto: null } }` so the merge service's soft-delete contract (loser keeps canonical key, gets `mergedInto` set + `isActive: false`) doesn't trip the constraint on rollback — a plain unique would refuse to build because winner+loser share canonical key after merge; (c) global `errorHandler.js` now maps `E11000` on `vip_client_name_clean` to a friendly 409 ("A VIP Client with this name already exists. Rename yours, or contact admin to share coverage.") so post-flip BDM UX never surfaces a raw field name; (d) new `backend/scripts/healthcheckMdMergeAudit.js` (CLAUDE.md note 12b TODO retired) verifies the 9-section contract end-to-end. **A.5.2 flip step (NOT YET RUN)**: as of May 04 2026 the dry-run shows **131 active duplicate groups** (predominantly Iloilo overlap — Jake Montero + Romela Shen Herrera covering the same MDs). Admin must merge them via [/admin/md-merge](frontend/src/pages/admin/MdMergePage.jsx) (**SHIPPED Apr 28 2026**) BEFORE running `node backend/scripts/migrateVipClientCanonical.js --add-unique-index` (the script refuses-on-active-dupes; this is the safety gate). `primaryAssignee` is the forward-compatible ownership scalar that A.5.4's `assignedTo` scalar→array flip will preserve. **Decision D11 (locked Apr 24, ratified by A.5.5)**: all A.5 lifecycle operations are gated by the `VIP_CLIENT_LIFECYCLE_ROLES` Lookup category (lazy-seed-from-inline-defaults, mirrors `PROXY_ENTRY_ROLES` in [resolveOwnerScope.js](backend/erp/utils/resolveOwnerScope.js) and `MD_PARTNER_ROLES` in [mdPartnerAccess.js](backend/utils/mdPartnerAccess.js)). Helper [backend/utils/resolveVipClientLifecycleRole.js](backend/utils/resolveVipClientLifecycleRole.js) exports getters for 7 codes: `VIEW_MERGE_TOOL`, `EXECUTE_MERGE`, `ROLLBACK_MERGE`, `HARD_DELETE_MERGED` (all wired by A.5.5), and `REASSIGN_PRIMARY`, `JOIN_COVERAGE_AUTO`, `JOIN_COVERAGE_APPROVAL` (forward-compat for A.5.4). Defaults: admin + president for VIEW/EXECUTE/ROLLBACK; **president-only** for HARD_DELETE_MERGED (it bypasses the 30-day rollback grace). Plan: `~/.claude/plans/phase-a5-canonical-vip-client.md`.
