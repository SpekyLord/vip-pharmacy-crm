...
21. **Phase A.5.3 + A.5.6 — Duplicate VIP Client UX + Phase-N Offline Merge Resolver** (May 06 2026, shipped — pending UI smoke ratification). **A.5.3**: structured `DUPLICATE_VIP_CLIENT` 409 + [DuplicateVipClientModal](frontend/src/components/admin/DuplicateVipClientModal.jsx) with three role-gated buttons (Rename mine / Join their coverage / Request admin approval). New `POST /api/doctors/:id/join-coverage` endpoint. **A.5.6**: offline-replayed visits walk `mergedInto` chain (cap 5 hops) BEFORE access/cap/schedule checks → response carries `merge_redirected: { from, to }`. Healthcheck: [healthcheckPhaseA5_3_6.js](backend/scripts/healthcheckPhaseA5_3_6.js) — 80/80 PASS. Full detail: [docs/PHASE-TASKS-CRM.md](docs/PHASE-TASKS-CRM.md) § Phase A.5.3 + A.5.6.
