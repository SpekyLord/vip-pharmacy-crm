...
16. **Phase P1.2 Slice 7-extension Round 2C — CWT-Inbound picker on Mark-Received modal** (May 06 2026, shipped + UI-ratified): closes the original Slice 7-extension scope. `Bir2307InboundPage.jsx` was deferred in Round 1 because the receive modal stored `cert_2307_url` as a typed string with no upload handler — wiring the picker would have meant a new upload route (estimated 4h). Round 2A's `skipFetch=true` mode dissolved the obstacle: the picker yields the bare S3 URL of the BDM-captured photo, which is exactly the shape `cert_2307_url` already accepts. ~3h thin-wrap on top of existing infra. **Wiring**: (a) `backend/erp/controllers/birController.markReceived2307Inbound` destructures optional `capture_id` from body + best-effort `linkCaptureToDocument(capture_id, 'CwtLedgerEntry', row._id, ctx)` after `cwt2307ReconciliationService.markReceived` succeeds — failures here log but do NOT roll back the receive (the CWT credit posture is the system of record; the back-link is audit metadata). Audit log line `[BIR_2307_INBOUND_MARK_RECEIVED]` extended with `from_capture: bool` + `capture_id` so admin can grep historical receives to see which were picker-sourced vs typed. (b) `frontend/src/erp/pages/Bir2307InboundPage.jsx` mounts `<PendingCapturesPicker>` inside the Mark-Received modal between the CR/hospital recap and the Certificate URL field with `workflowTypes=['CWT_INBOUND', 'UNCATEGORIZED']`, **cross-BDM scope (`bdmId={null}`)** because the cert may have been snapped by office runner / receptionist / finance herself — not the BDM who originated the underlying sale, so per-BDM filter would hide legitimate captures. `skipFetch={true}` + `maxSelect={1}`. New `pickedCaptureId` state + `onPickFromCaptures` callback writes `cert_2307_url` from `meta.captures[0].captured_artifacts[0].url`, auto-fills `cert_filename` from artifact key tail, and stores `capture_id` for the eventual receive submit. **Manual edit of the URL field after picking clears `pickedCaptureId`** — defense in depth so a typo doesn't silently re-stamp an unrelated capture against a hand-typed URL. Modal close + receive success both clear pickedCaptureId; "✓ Linked to BDM Capture" pill renders below the picker when one is selected. (c) `frontend/src/components/common/PageGuide.jsx` `bir-2307-inbound` banner reflects the new flow (Rule #1) — workflow step now reads "either pulls the photo from BDM Captures (Round 2C — see step 4) OR types where they saved the file"; new step 4 walks the picker UX + cross-BDM scope rationale + lifecycle advance; subscription-readiness step lists `CAPTURE_LIFECYCLE_ROLES.PROXY_PULL_CAPTURE` alongside `BIR_ROLES.RECONCILE_INBOUND_2307`. **Lifecycle**: `CWT_INBOUND` is in `REVIEW_WORKFLOWS` (Slice 1 baseline) so picked captures advance `PENDING_PROXY → AWAITING_BDM_REVIEW` rather than auto-`PROCESSED` — same posture as Sales / Expense / Fuel / Collection captures. `CwtLedgerEntry` was already in `linked_doc_kind` enum on `CaptureSubmission`. **Subscription-ready**: zero new lookups. Reuses `CAPTURE_LIFECYCLE_ROLES.PROXY_PULL_CAPTURE` + `BIR_ROLES.RECONCILE_INBOUND_2307`; subscribers retune via Control Center → Lookup Tables, no code deploy (Rule #3 / Rule #19). **Defer status update**: SMER (`Smer.jsx`) stays deferred indefinitely — digital-only, no photo-upload UI to mount a picker next to. The right consumer for SMER captures is Slice 6 Car Logbook auto-populate (ODO capture's OCR reading flows into `starting_km`/`ending_km`). **Healthcheck**: [backend/scripts/healthcheckCaptureHub.js](backend/scripts/healthcheckCaptureHub.js) Section 16 — 24 assertions covering page wiring + manual-edit invalidation + picker mount + capture_id forward + modal close hygiene + backend wiring + model enum + PageGuide + lookup gate + PHASETASK-ERP doc; Section 13 defer-rationale assertion for Bir2307Inbound dropped, SMER assertion stays. **PHASETASK-ERP defer-rationale heading rewritten**: "SMER" only (was "SMER + CWT-Inbound"); new "Phase P1.2 Slice 7-extension Round 2C" section above Round 2A.

17. **Phase P1.2 Slice 6 — Car Logbook Auto-Populate** (May 06 2026, shipped + UI-ratified): closes the last Phase P1.2 piece. `/erp/car-logbook` is now a *review surface* — proxies open the page and review a pre-populated grid built from four signals instead of typing the same data twice. **Sources**: (a) `SmerEntry.daily_entries.hospital_covered + notes` → `destination`; (b) `CaptureSubmission` `workflow_type='SMER'` ODO photos with OCR-extracted `reading` → min reading = `starting_km` (with photo URL), max reading = `ending_km` (with photo URL); (c) `DriveAllocation` row → `personal_km` / `official_km`; (d) `CaptureSubmission` `workflow_type='FUEL_ENTRY'` with OCR `extracted.{station_name,fuel_type,liters,price_per_liter,total_amount,date}` → `fuel_entries[]` per artifact; plus a `PRIOR_DAY` fallback for `starting_km` when no SMER ODO captured today (most-recent VALID/POSTED `CarLogbookEntry.ending_km` strictly before today). **Wiring**: new [backend/erp/services/carLogbookAutoPopulate.js](backend/erp/services/carLogbookAutoPopulate.js) — `autoPopulateCarLogbookDay({entity_id, bdm_id, entry_date})` returns the populated row + `_autopop_sources` provenance map + `_smer_meta` + `_drive_allocation` raw signals + `_has_any_signal` boolean; all four pulls run in parallel via `Promise.all`; helpers `pullSmerDestination` / `pullSmerOdoCaptures` / `pullDriveAllocation` / `pullFuelEntries` / `pullPriorDayEndingKm` are testable pure functions; `manilaDayBounds(dateStr)` returns Manila-day UTC bounds (midnight = UTC midnight − 8h); `readOcrNumber` / `readOcrString` coerce the three known OCR shapes (raw, `{value,confidence}` envelope, string with units/commas) — empty after strip → null so `'garbage'` doesn't masquerade as 0. New `GET /erp/expenses/car-logbook/preview` endpoint in [expenseController.js](backend/erp/controllers/expenseController.js) supports `?bdm_id=X&date=YYYY-MM-DD` (single) and `?bdm_id=X&dates=...` (batch, max 31 days/call); enforces Rule #21 — privileged callers MUST pass `bdm_id`, HTTP 400 otherwise. `createCarLogbook` extended with opt-in `autopopulate=true` (query OR body); body wins on overlapping fields (proxy's manual override is the winning state); auto-populate is non-fatal (logs warn + falls through to plain create); per-field provenance persists in `edit_history[0] = {action: 'AUTO_POPULATE', sources, at, by}`. Route mounted **BEFORE** `/car-logbook/:id` so Express resolves the literal (Slice 8 shadowing pattern). Frontend [CarLogbook.jsx](frontend/src/erp/pages/CarLogbook.jsx) renders a new `SourceBadge` component (mini pill, 6 colors + tooltips) next to destination / start_km / end_km / personal_km / fuel_entries; `loadAndMerge` calls `previewCarLogbookDay` once per cycle for days WITHOUT a saved row, fills only blanks (saved rows untouched), falls back silently to legacy SMER-destinations-only path if preview fails (older deployments without Slice 6 keep rendering); `handleRowChange` flips the source tag to `'MANUAL'` (red badge) on edit; `saveRow` sends `autopopulate: true` for defense-in-depth (server re-runs the merge in case the in-memory `_autopop` state drifted). New [hook method previewCarLogbookDay({bdmId, date, dates})](frontend/src/erp/hooks/useExpenses.js) builds query params for both shapes from one method. **Banner update** ([WorkflowGuide.jsx](frontend/src/erp/components/WorkflowGuide.jsx) `'car-logbook'`, Rule #1) — new step explaining the 4 sources + badge palette; tip extended with the full Slice 6 overview including the lookup-driven `EDIT_CAR_LOGBOOK_DESTINATION` gate. **Subscription readiness**: every helper sweep is `entity_id`-scoped (Rule #19); `EDIT_CAR_LOGBOOK_DESTINATION` lookup gate (Slice 1, default `[admin, finance, president]`) covers destination edits; `bdm_id` explicit on every preview call (Rule #21). The `CarLogbookEntry` pre-save hook still owns derived math (total_km, official_km, fuel total_amount, efficiency, gas split) — auto-populate writes only the inputs. **Healthcheck**: new [backend/scripts/healthcheckCarLogbookAutoPopulate.js](backend/scripts/healthcheckCarLogbookAutoPopulate.js) — **70/70 PASS**. Existing [healthcheckCaptureHub.js](backend/scripts/healthcheckCaptureHub.js) still **531/531 PASS** (no Slice 1+2+3+4+5+7+8+9 regression). Service helper unit smoke (Bash `node -e`): `readOcrNumber` 8 cases + `readOcrString` 5 cases + `manilaDayBounds` start/end/garbage = **16/16 PASS**. **Vite build**: ✓ 35.55s, no warnings. **Live API smoke** as president on dev: `GET /preview?date=...` (no bdm_id) → 400 with friendly Rule #21 message; `?bdm_id=Mae&dates=...` → 200 with Apr 30 carrying `_autopop_sources.personal_km='DRIVE_ALLOCATION'`, `personal_km: 75`, `official_km: 25`, `_drive_allocation` raw signal included. **Live Playwright UI smoke** as president on dev: `/erp/car-logbook` → BDM picker = Mae Navarro → period 2026-04 → cycle C2 → preview fetched once for the 11 cycle workdays → 17-row grid → Apr 30 row shows green "Drive" badge with tooltip "Filled from BDM's DriveAllocation row (Personal/Official slider)" → typed "50" into personal_km → badge flips to red "Manual" with tooltip "Manual override — proxy edited this field" + unsaved `*` indicator. Screenshot residue at repo root: `phase-p1-2-slice-6-carlogbook-autopop-ratified.png`, `phase-p1-2-slice-6-manual-override-ratified.png` (do NOT commit). **Files modified/added (7)**: `backend/erp/services/carLogbookAutoPopulate.js` (NEW), `backend/erp/controllers/expenseController.js` (preview endpoint + createCarLogbook autopopulate merge + import), `backend/erp/routes/expenseRoutes.js` (+1 route before `/:id`), `backend/scripts/healthcheckCarLogbookAutoPopulate.js` (NEW), `frontend/src/erp/hooks/useExpenses.js` (+1 hook method), `frontend/src/erp/pages/CarLogbook.jsx` (+SourceBadge + AUTOPOP_SOURCE_META + preview wiring + MANUAL flip + autopopulate=true on save), `frontend/src/erp/components/WorkflowGuide.jsx` (banner + tip). Plus 3 docs: PHASETASK-ERP.md, CLAUDE-ERP.md, this CLAUDE.md note.

18. **Phase P1.2 Slice 6.1 — CRM Visit cities also auto-fill Car Logbook destination** (May 06 2026 evening, shipped + UI-ratified): closes the architectural mismatch from Slice 6 — pre-Slice-6.1, Car Logbook's destination cell only populated from SMER's `hospital_covered + notes`, which meant if the BDM hadn't clicked "Pull from CRM" on the SMER page yet, the destination stayed empty even when CRM Visit data was sitting right there. New `pullCrmVisitDestinations({bdm_id, dateStr})` helper in [carLogbookAutoPopulate.js](backend/erp/services/carLogbookAutoPopulate.js) queries `Visit + ClientVisit` for the day (yes-equal-weight VIP + EXTRA per smerCrmBridge contract), joins `Doctor.locality + .province` (Phase G1.5 backfill — every record has structured fields; falls back to `clinicOfficeAddress` for legacy doctors), dedupes the day's unique localities via `Set`, joins with `'; '`. New `SOURCE_TAGS.CRM_VISIT_CITY` (cyan badge `#cffafe`/`#155e75`, label "CRM"). **Destination resolution chain** in `autoPopulateCarLogbookDay`: CRM Visits > SMER hospital_covered > empty (CRM wins because it's canonical "where the BDM physically went" — typing-free, decoupled from BDM's SMER timing). The user's mental model "I know the territory just by looking the city, town or province" is the design rationale — no Haversine / Google Maps Distance Matrix tier shipped (out of scope per user direction). **Wiring**: helper added to the 5-way `Promise.all` parallel sweep alongside SMER/ODO/Drive/Fuel; `_crm_visits` raw signal surfaced on the return shape so the UI can show drill-down later if desired; pure additive — `pullSmerDestination` untouched, SMER's notes auto-fill from CRM (Smer.jsx:550) unchanged. Frontend [CarLogbook.jsx:110-117](frontend/src/erp/pages/CarLogbook.jsx) `AUTOPOP_SOURCE_META` extended 6 → 7 codes with cyan CRM entry + tooltip "Filled from CRM Visits — locality + province of every doctor/client the BDM visited that day". **Banner update** ([WorkflowGuide.jsx](frontend/src/erp/components/WorkflowGuide.jsx) Rule #1) — `car-logbook` step 2 + tip extended with FIVE-source narrative + CRM > SMER priority + cyan badge mention; `smer` tip appended with Slice 6.1 cross-fill note. **Subscription readiness**: zero new lookup categories — re-uses `Doctor.locality + .province` (Phase G1.5 schema). The new helper does NOT scope by `entity_id` — Visit/ClientVisit are CRM models scoped by `user` (the BDM), so when SaaS spin-out generalizes `entity_id` to `tenant_id` (Year 2 per CLAUDE.md 0d), this helper inherits whatever scoping the CRM Visit collection ships with at that time. `bdm_id` required at the controller (Rule #21 — `previewCarLogbookDay` returns 400 if privileged caller omits it; preserved). **Healthcheck**: [healthcheckCarLogbookAutoPopulate.js](backend/scripts/healthcheckCarLogbookAutoPopulate.js) extended with 17 new assertions — **88/88 PASS** (was 70/70 pre-Slice-6.1). [healthcheckCaptureHub.js](backend/scripts/healthcheckCaptureHub.js) regression suite still **576/576 PASS** (no Slice 1+2+3+4+5+6+7+8+9 regression). **Vite build**: ✓ green in 13.44s, no warnings. **Live API smoke** (Mae Navarro on dev as president): `GET /api/erp/expenses/car-logbook/preview?bdm_id=Mae&date=2026-05-03` → 200 + `destination: "BAMC; TDh; Medical Plaza; San carlos city"` + `_autopop_sources.destination: "CRM_VISIT_CITY"` + `_crm_visits: { destination: ..., visit_count: 7 }`; `?date=2026-05-05` → 200 + `"BAMC; Medical Plaza"` + same source tag + visit_count 2; `?date=2026-04-30` → 200 + empty destination + null source (no CRM data, correctly falls through); Rule #21 — privileged caller without `?bdm_id=` → HTTP 400 with friendly "bdm_id is required — privileged/proxy users must specify which BDM to preview". **Live Playwright UI smoke** (president on `/erp/car-logbook` → Mae Navarro): banner step 2 renders the new "FIVE sources" / "cyan CRM badge" text; May 5 row destination cell shows `"BAMC; Medical Plaza"` + cyan `CRM` badge with correct tooltip; typing "Custom proxy override" into the cell flips badge to red `Manual` (`#fee2e2`/`#991b1b`) per Slice 6 contract. 0 console errors throughout. Screenshot residue at repo root: `phase-p1-2-slice-6-1-crm-cities-ratified.png` (do NOT commit). **Files modified (5)**: `backend/erp/services/carLogbookAutoPopulate.js` (+4 model imports + SOURCE_TAG + helper + 5-way Promise.all + dest resolution chain + return shape), `frontend/src/erp/pages/CarLogbook.jsx` (+CRM_VISIT_CITY entry in AUTOPOP_SOURCE_META), `frontend/src/erp/components/WorkflowGuide.jsx` (car-logbook step 2 + tip + smer tip), `backend/scripts/healthcheckCarLogbookAutoPopulate.js` (+17 assertions). Plus 3 docs: PHASETASK-ERP.md (new Slice 6.1 section), CLAUDE-ERP.md (status banner refresh to v8.7), this CLAUDE.md note. **Out of scope** (intentionally not done per user direction "I do not need the estimated km"): Tier 2 Haversine straight-line distance, Tier 3 Google Maps Distance Matrix road distance.

19. **Phase P1.2 Slice 6.2 — GRN sub_type split (BATCH_PHOTO vs WAYBILL)** (May 06 2026, shipped — pending UI smoke ratification): the single `Scan GRN Item` tile on `/erp/capture-hub` (Truck icon, "Product barcode + qty + batch/expiry + waybill") is replaced by **two** tiles in the `inventory` section so the proxy can tell at-a-glance which captures need a paper hand-in vs which are pure OCR feedstock. **(D)** `Scan Batch Photo` (sub_type=`BATCH_PHOTO`, ScanBarcode lucide icon, amber `#d97706`, `digitalOnly=true`, `artifactKind='barcode_scan'`) — photo of vial/box labels for OCR batch + expiry; the physical product itself is the source so no paper arrives at office; backend stamps `physical_required=false` / `physical_status='N_A'`, Slice 3 reconciliation skips. **(M)** `Scan Waybill` (sub_type=`WAYBILL`, Truck icon, red `#dc2626`, `artifactKind='photo'`) — courier waybill paper; physical paper expected at office; backend stamps `physical_required=true` / `physical_status='PENDING'` and the cycle reconciliation gate enforces hand-in. **Schema** ([backend/erp/models/CaptureSubmission.js](backend/erp/models/CaptureSubmission.js)) — `sub_type` enum gains `'BATCH_PHOTO'` + `'WAYBILL'` alongside the legacy `'CR' / 'DEPOSIT' / 'PAID_CSI'`. **Controller** ([backend/erp/controllers/captureSubmissionController.js](backend/erp/controllers/captureSubmissionController.js)) — flat `VALID_SUB_TYPES = ['CR','DEPOSIT','PAID_CSI']` (which hardcoded a `workflow_type !== 'COLLECTION'` rejection) → per-workflow map `VALID_SUB_TYPES_BY_WORKFLOW = { COLLECTION: [...3], GRN: ['BATCH_PHOTO','WAYBILL'] }`. `DIGITAL_ONLY` arrow gains a third branch `(workflow_type === 'GRN' && sub_type === 'BATCH_PHOTO')` joining `'SMER'` and `('COLLECTION' && 'PAID_CSI')`. The 400 message now lists the per-workflow allowed values (`Expected one of: BATCH_PHOTO, WAYBILL`). Adding a third sub-typed workflow in the future is a one-row map insertion, not a controller rewrite. **Frontend** ([BdmCaptureHub.jsx](frontend/src/erp/pages/mobile/BdmCaptureHub.jsx)) — `ScanBarcode` lucide import added; legacy single GRN tile entry replaced with two tile entries; existing `payload.sub_type = workflow.sub_type` line forwards the sub_type to the API unchanged; existing composite key generator `${w.key}_${w.sub_type || 'main'}` produces unique React keys (`GRN_BATCH_PHOTO`, `GRN_WAYBILL`) without code change. **Migration** ([backend/scripts/migrateGrnCaptureSubType.js](backend/scripts/migrateGrnCaptureSubType.js), NEW) — dry-run-by-default, idempotent; sets `sub_type='WAYBILL'` on every legacy GRN row whose `sub_type` is `null` or missing. Conservative-by-design: WAYBILL preserves the legacy `physical_required=true` posture exactly. Per-row corrections (a row that was actually a batch photo) flow through the existing `OVERRIDE_PHYSICAL_STATUS` path on Capture Archive — no special tooling needed. **Banner** ([WorkflowGuide.jsx](frontend/src/erp/components/WorkflowGuide.jsx) `bdm-capture-hub` Rule #1) — new tip explaining the BATCH_PHOTO digital-only / WAYBILL paper-expected split. **Healthcheck** ([backend/scripts/healthcheckCaptureHub.js](backend/scripts/healthcheckCaptureHub.js)) — new "Phase P1.2 Slice 6.2" section: 17 assertions covering model enum + controller per-workflow map + DIGITAL_ONLY arrow body precise check (regex extracts the arrow body and asserts BATCH_PHOTO branch present + WAYBILL branch absent — the WAYBILL token correctly appears later in the file in the `VALID_SUB_TYPES_BY_WORKFLOW` map but NOT inside the arrow body) + 7 frontend tile assertions + 5 migration script assertions + 1 banner assertion. Renames `'sub_type field with 3 values'` → `'sub_type field with COLLECTION trio'`, adds `'sub_type field with GRN pair'`. Renames `'VALID_SUB_TYPES has all three'` and `'sub_type rejected for non-COLLECTION'` to per-workflow assertions. **Subscription readiness**: zero new lookup categories — sub_type whitelist stays inline (matches the legacy posture for COLLECTION sub_types). Rule #3 future hook documented in the controller comment: subscribers who want to flip a sub_type's D↔M classification per-entity (e.g., a subscriber whose courier process digitizes the waybill via PDF email instead of paper) can introduce a `CAPTURE_SUB_TYPE_DIGITAL_OVERRIDES` lookup category later — not needed for any known subscriber today. Sub-permission gates unchanged: both new tiles inherit `CAPTURE_LIFECYCLE_ROLES.UPLOAD_OWN_CAPTURE` (default `[staff]`) for BDM-side capture and `PROXY_PULL_CAPTURE` for office-side reuse via the PendingCapturesPicker. `entity_id` (Rule #19) and `bdm_id` (Rule #21) posture unchanged. **Cascading downstream** — Slice 3 reconciliation index `{bdm_id, physical_status, created_at}` unchanged; BATCH_PHOTO writes `physical_status='N_A'` so the sweep correctly skips. Slice 8 Capture Archive aggregation already groups by `sub_type` so BATCH_PHOTO + WAYBILL automatically split into separate folders within the GRN workflow leaf. Slice 9 paper-received attestation gate is `if (paper_received === true && doc.physical_required)` — BATCH_PHOTO silently ignored (correct), WAYBILL attested as before. Slice 9 partial `linkCaptureToDocument` walks `PENDING_PROXY → AWAITING_BDM_REVIEW` only for `REVIEW_WORKFLOWS` which does NOT include GRN — both new sub_types continue to land in `PROCESSED` when attached to a GRN doc (correct posture, GRN already has UT-acknowledgment cascade). PendingCapturesPicker on `/erp/grn` continues to fetch `workflowTypes={['GRN','UNCATEGORIZED']}` — picker chip already differentiates BATCH PHOTO vs WAYBILL. Recent Captures list on `/erp/capture-hub` already renders `sub_type.replace(/_/g, ' ')` so post-migration rows display "GRN · WAYBILL" / "GRN · BATCH PHOTO". **Files modified/added (6)**: `backend/erp/models/CaptureSubmission.js`, `backend/erp/controllers/captureSubmissionController.js`, `backend/erp/scripts/...` not used (script is `backend/scripts/migrateGrnCaptureSubType.js` NEW), `backend/scripts/healthcheckCaptureHub.js` (Phase Slice 6.2 section), `frontend/src/erp/pages/mobile/BdmCaptureHub.jsx`, `frontend/src/erp/components/WorkflowGuide.jsx`. Plus 3 docs: `CLAUDE-ERP.md` (status banner refresh to v8.8), `docs/PHASETASK-ERP.md` (new Slice 6.2 section above Slice 6.1), this CLAUDE.md note. **Out of scope** (intentionally NOT done): picker filter tightening (narrowing `/erp/grn` PendingCapturesPicker to BATCH_PHOTO only is a Phase 2.1 follow-up); subscriber-overrideable D/M flag per sub_type (no subscriber needs this today); WAYBILL upload field on `/erp/grn` accepting WAYBILL captures via picker (Phase 2.1 follow-up).

20. **Phase P1.2 Phase 1 — Capture-type cleanup + Option D BIR audit gate** (May 06 2026, shipped — pending UI smoke ratification): three coupled changes that close an audit-credibility gap exposed by Round 2C and clean up two unused enum slots. **(1)** `workflow_type='CWT_INBOUND'` is dropped — CWT now lives as a sub_type of `COLLECTION` (hospitals send CR + DEPOSIT + CWT together as one collection package, so a single workflow_type covers them all). The Capture Hub tile flips from `key:'CWT_INBOUND'` to `key:'COLLECTION', sub_type:'CWT'`; composite tile key resolves to `COLLECTION_CWT` (vs legacy `CWT_INBOUND_main`). **(2)** `workflow_type='PETTY_CASH'` is dropped — tile was never shipped on the Capture Hub; enum slot was speculative. ProxyQueue dropdown drops the PETTY_CASH option, adds COLLECTION (with the inline sub_type hint) + UNCATEGORIZED. **(3) Two-step BIR receive on `/erp/bir/2307-IN` (Option D audit gate)** — photo evidence (`cert_2307_url`) attaches but does NOT unlock the 1702 credit; finance must ALSO tick a "I confirm the paper certificate is in the Iloilo office archive" checkbox that flips `status='RECEIVED'` + stamps a NEW `CwtLedger.physical_received_at` field. The gap state (cert URL non-empty + status=PENDING_2307 + physical_received_at NULL) is the new **PHOTO_ATTACHED** pseudo-status, surfaced as a tab between Pending and Received + an amber pill (`#fef3c7`/`#92400e`) on the inbound page. **Why the gate matters:** BIR RR No. 2-98 + 1604-E line-item docs require the original paper certificate as documentary evidence for the 1702 "Less: Creditable Tax Withheld" credit. Round 2C let finance pick a BDM photo and immediately mark RECEIVED → 1702 credit unlocked, which under audit could be disallowed → 25% surcharge + 12% interest on the underpaid amount. CSI/CR don't carry this same one-way risk because they're internal docs VIP issues; CWT is the only doc VIP RECEIVES from a third party AND CLAIMS A TAX CREDIT against. **Migration ([backend/scripts/migrateCwtInboundToCollectionSubtype.js](backend/scripts/migrateCwtInboundToCollectionSubtype.js))** — dry-run-by-default, idempotent, raw-collection access (works regardless of model enum state). 2 CWT_INBOUND rows flipped on dev cluster, 0 PETTY_CASH. Migration MUST run before model enum is narrowed (otherwise existing rows fail Mongoose validation on next save — same posture as `migrateGrnCaptureSubType.js`). **Backend wiring**: [CwtLedger.js](backend/erp/models/CwtLedger.js) gains `physical_received_at: { type: Date, default: null, index: true }` — distinct from existing `received_at` (the digital-attach timestamp). Indexed because the new PHOTO_ATTACHED tab filters rows where `cert_2307_url IS NOT NULL && physical_received_at IS NULL`. [cwt2307ReconciliationService.markReceived](backend/erp/services/cwt2307ReconciliationService.js) signature gains `paper_received` (default `true` for backward compat with non-Phase-1 callers; Phase-1 frontend always passes explicitly); cert_* fields always stamped, paper attestation only when `paper_received===true`; rejects `paper_received=true` with empty `cert_2307_url` (defense-in-depth mirror of disabled-checkbox UI gate); photo-only saves on already-RECEIVED rows preserve the prior paper attestation (don't demote). `markPending` clears `physical_received_at` along with `received_at`/`received_by`. `listInboundRows` surfaces `physical_received_at` so the frontend can render PHOTO_ATTACHED. [birController.markReceived2307Inbound](backend/erp/controllers/birController.js) extracts `paper_received` from body, coerces with backward-compat default `true` if absent, forwards to service. Audit log line `[BIR_2307_INBOUND_MARK_RECEIVED]` extended with `paper_received` + `final_status` + `physical_received_at` so admin can grep historical events to see which receives unlocked the 1702 credit (`paper_received:true`) vs which only attached photo evidence (`paper_received:false → row stays in PHOTO_ATTACHED`). [captureSubmissionController.js](backend/erp/controllers/captureSubmissionController.js) `VALID_TYPES` drops `CWT_INBOUND` + `PETTY_CASH`; `VALID_SUB_TYPES_BY_WORKFLOW.COLLECTION` adds `'CWT'` (now 4 values: CR / DEPOSIT / PAID_CSI / CWT); `REVIEW_WORKFLOWS` (in both `completeCapture` AND `linkCaptureToDocument`) drops `CWT_INBOUND` (COLLECTION already covers it via sub_type); `getProxyQueue` accepts new optional `sub_type` query param + threads to the Mongo filter; `DIGITAL_ONLY` arrow does NOT classify COLLECTION+CWT as digital-only — paper certificate must arrive at the Iloilo office, so `physical_required=true` is correct. **Frontend wiring**: [PendingCapturesPicker.jsx](frontend/src/erp/components/PendingCapturesPicker.jsx) gains a new `subTypeFilter` prop (Object keyed by workflow_type → sub_type, e.g. `{COLLECTION:'CWT'}`); when a workflow_type matches a key in the map, picker passes `sub_type` to the queue; UNCATEGORIZED in the same picker iterates without narrowing. `useCallback` dependencies include `subTypeFilter` so a parent prop change re-fetches. [Bir2307InboundPage.jsx](frontend/src/erp/pages/Bir2307InboundPage.jsx) gains `paperReceived` state (default UNCHECKED on fresh open; pre-ticked when reopening already-attested row), the new "Photo attached" tab between Pending and Received (counts via `tabCounts` computed off effective-status, not server `status` — so PHOTO_ATTACHED rows correctly leave the Pending bucket), the paper-received checkbox in the Mark-Received modal (disabled until cert URL non-empty, cites BIR RR No. 2-98 in the helper text, palette flips green when checked), Save button label branches on `paperReceived` (`'Save — Mark RECEIVED'` checked vs `'Save Photo Only'` unchecked) + Save button is disabled until cert URL non-empty + Save guard rejects `paperReceived && !cert_2307_url` before API call (defense in depth), `STATUS_META.PHOTO_ATTACHED` row pill, `effectiveStatus(row)` helper. Picker workflowTypes flip from `['CWT_INBOUND','UNCATEGORIZED']` → `['COLLECTION','UNCATEGORIZED']` + new `subTypeFilter={{COLLECTION:'CWT'}}` prop. [BdmReviewQueue.jsx](frontend/src/erp/pages/mobile/BdmReviewQueue.jsx) WORKFLOW_ICONS / WORKFLOW_COLORS / WORKFLOW_LABELS get a new `COLLECTION_CWT` composite-key entry (FileBadge / `#6366f1` / `'CWT (BIR 2307)'`); legacy `CWT_INBOUND` + `PETTY_CASH` entries kept as defensive fallbacks for any pre-migration row. [CaptureArchive.jsx](frontend/src/erp/pages/capture/CaptureArchive.jsx) inherits the existing composite-key `workflowLabel("COLLECTION", "CWT")` → `"COLLECTION / CWT"` rendering; legacy CWT_INBOUND keys retained as fallback. **Cascading downstream sanity verified**: Phase J7 1702 aggregator reads `status='RECEIVED'` so it correctly excludes PHOTO_ATTACHED rows from the credit (their `status` is still `'PENDING_2307'`); pending-exposure computation correctly counts photo-attached rows as exposure (they ARE still credit-at-risk until paper arrives); J6 inbound posture card splits by `status` so PHOTO_ATTACHED rows count under PENDING_2307; Capture Archive (Slice 8) folder tree groups by `(workflow_type, sub_type)` so post-migration CWT captures appear under `COLLECTION / CWT` (vs the legacy `CWT_INBOUND` standalone leaf); Slice 9 paper-received attestation gate lives on `CaptureSubmission.physical_received_at` (capture-side), distinct from `CwtLedger.physical_received_at` (CWT-ledger-side) — the two paper-received fingerprints serve different surfaces and don't interact; Round 2C `linkCaptureToDocument` still fires when `capture_id` is forwarded, regardless of `paper_received` value (the capture lifecycle advance is independent of the BIR audit gate). **Subscription-readiness preserved**: zero new lookup categories. Paper-attestation gate is hardcoded by BIR-regulation requirement, not subscriber-tunable. Picker reuses `CAPTURE_LIFECYCLE_ROLES.PROXY_PULL_CAPTURE` (defaults `[admin, finance, president]`) + `BIR_ROLES.RECONCILE_INBOUND_2307` (defaults `[admin, finance, bookkeeper]`); subscribers retune via Control Center → Lookup Tables (Rule #3 / Rule #19). `entity_id` (Rule #19) and `bdm_id` (Rule #21) posture unchanged on every endpoint touched. **Banner update** ([PageGuide.jsx](frontend/src/components/common/PageGuide.jsx) `bir-2307-inbound`, Rule #1) — 8-step rewrite covers pre-flight → workflow → tabs → TWO-STEP RECEIVE (Option D explanation) → picker workflow_type=COLLECTION → exposure number → audit trail (paper-attest fingerprint distinguishes "we have a digital reference" vs "we have the binding paper original") → role gates → EXCLUDE; new tip explains "Photo attached" tab as the weekly chase queue. **Healthcheck** ([healthcheckCaptureHub.js](backend/scripts/healthcheckCaptureHub.js)) — new Section 16b "Phase P1.2 Phase 1" (~50 asserts) covering the model + service + controller + picker + page contract end-to-end; Section 1 / 2 / 3 / 4 / Round 2C asserts updated for narrowed enum + COLLECTION_CWT composite key + new picker workflowTypes / subTypeFilter shape. **Files modified/added (16)**: 8 backend (`CaptureSubmission.js`, `CwtLedger.js`, `captureSubmissionController.js`, `cwt2307ReconciliationService.js`, `birController.js`, `migrateCwtInboundToCollectionSubtype.js` NEW, `healthcheckCaptureHub.js`) + 5 frontend (`PendingCapturesPicker.jsx`, `BdmCaptureHub.jsx`, `BdmReviewQueue.jsx`, `CaptureArchive.jsx`, `ProxyQueue.jsx`, `Bir2307InboundPage.jsx`, `PageGuide.jsx`) + 3 docs (`CLAUDE-ERP.md` v8.9 banner refresh, `docs/PHASETASK-ERP.md` new Phase 1 section, this CLAUDE.md note 20). **Out of scope** (deferred): GRN picker filter tightening (Slice 6.2 follow-up); OPENING_AR cleanup (deferred until April 1 CSI backfill is complete — Phase 3); BLANK_CSI / PINK_CSI sub_type split (current SALES workflow_type without discriminator is acceptable conflation).

21. **Phase A.5.3 + A.5.6 — Duplicate VIP Client UX + Offline Merge Resolver** (May 06 2026, shipped — pending UI smoke ratification): closes the dedup story opened by A.5.1 + A.5.2 + A.5.4 + A.5.5. Two coupled concerns, one merged commit. **A.5.3 (Duplicate-on-Create UX)**: creating or renaming a VIP Client whose canonical key (`lastname|firstname` lowercased) already exists used to surface as a generic 409 toast. Now the backend returns a structured `DUPLICATE_VIP_CLIENT` 409 with the colliding record's identity + visit count + role-gated `can_join_auto` / `can_join_approval` flags, and the admin frontend renders [DuplicateVipClientModal](frontend/src/components/admin/DuplicateVipClientModal.jsx) on top of the add/edit modal (z-index 1100 > 1000) offering three buttons: **Rename mine** (always shown, closes only the duplicate modal so admin can adjust + retry), **Join their coverage** (`$addToSet: { assignedTo: req.user._id }` against the existing record, gated by `VIP_CLIENT_LIFECYCLE_ROLES.JOIN_COVERAGE_AUTO` — defaults `[admin, president]`), or **Request admin approval** (posts a `MessageInbox` row to all admins with `category='approval_request'` + the BDM's optional notes — gated by `VIP_CLIENT_LIFECYCLE_ROLES.JOIN_COVERAGE_APPROVAL`, default `[admin, president]`; subscribers loosen to include `staff` so BDMs can request without admin password-share). New endpoint `POST /api/doctors/:id/join-coverage` ([backend/routes/doctorRoutes.js](backend/routes/doctorRoutes.js)) wraps both flows; the controller branch decides AUTO vs APPROVAL_PENDING by re-reading the lookup, so admin's lookup edits propagate within the helper's 60s TTL window without app restart (Rule #19 hot-config). The endpoint is idempotent on already-assigned (returns 200 + `already_assigned: true`) and rejects merged-loser refs (409 + `code: 'DOCTOR_MERGED'` + `mergedInto` so the client can refresh and retry on the winner). Created/updated controllers ([doctorController.js](backend/controllers/doctorController.js)) wrap `Doctor.create` + `doctor.save()` in try/catch keyed off the new `isVipClientNameCleanDuplicate(err)` predicate (handles `keyPattern` / `keyValue` / `errmsg` driver-version drift); the structured 409 builder `buildDuplicateVipClient409(req, key)` re-fetches the existing Doctor with `assignedTo` + `primaryAssignee` populated and `Visit.countDocuments({doctor: existing._id})` to power the modal card. Both gates resolve via `userCanPerformLifecycleAction` ([resolveVipClientLifecycleRole.js](backend/utils/resolveVipClientLifecycleRole.js)) which now exposes its previously forward-compat `JOIN_COVERAGE_AUTO` + `JOIN_COVERAGE_APPROVAL` getters as live consumer-side. The global `errorHandler.js` 409 fallback stays in place as defense-in-depth for any non-modal caller (e.g. legacy curl scripts, future BDM-side editors). DoctorsPage ([handleSaveDoctor + performUpgrade](frontend/src/pages/admin/DoctorsPage.jsx)) intercept 409 with `code === 'DUPLICATE_VIP_CLIENT'` and open the modal — the upgrade-Regular-to-VIP path also intercepts so `clientService.delete(regularClient._id)` does NOT fire on duplicate (the regular client stays in place; admin can retry rename or join). **A.5.6 (Phase-N Offline Merge Resolver)**: A BDM working offline still has a doctor list cached on-device. While they're offline, admin runs the merge tool and consolidates duplicates — that BDM's doctorId now points at a soft-deleted loser (`mergedInto != null`, `isActive: false`). When their offline queue replays the visit, the resolver in [visitController.createVisit](backend/controllers/visitController.js) walks the `mergedInto` chain (capped at `MAX_MERGE_HOPS=5` for cycle defense) BEFORE the access check / weekly-cap check / schedule match, redirecting `doctorId` to the canonical winner so the visit lands on the right record. Without this, the visit would create a row attributed to the merged loser and become invisible to every list query (loser is `isActive:false`). Response carries `merge_redirected: { from, to, message }` so the frontend can show a soft info toast ("This VIP Client record was consolidated. The visit was logged against the canonical record."). The redirect is logged at `[Phase A.5.6]` for ops visibility. **Subscription-readiness**: zero new lookup categories — re-uses the A.5.4-shipped `VIP_CLIENT_LIFECYCLE_ROLES.JOIN_COVERAGE_AUTO` + `JOIN_COVERAGE_APPROVAL` rows already seeded in [lookupGenericController.js SEED_DEFAULTS](backend/erp/controllers/lookupGenericController.js) with `insert_only_metadata: true` so admin tweaks survive future re-seeds. Defaults: `[admin, president]` for both. Subscribers loosen `JOIN_COVERAGE_AUTO` to include `staff` for instant BDM self-join, OR loosen only `JOIN_COVERAGE_APPROVAL` to give BDMs the "request approval" button instead of admin self-service. **Banner update** ([PageGuide.jsx](frontend/src/components/common/PageGuide.jsx) `doctors-page`, Rule #1) — new step 7 explains the "Rename mine / Join their coverage" UX; tip references both Phase A.5.3 and A.5.6 by name, names the lookup category + 2 codes so admin discovers tunability. **Healthcheck**: [backend/scripts/healthcheckPhaseA5_3_6.js](backend/scripts/healthcheckPhaseA5_3_6.js) NEW — 11 sections, **80/80 PASS**: Doctor model (8 asserts), errorHandler fallback (3), doctorController contract (21), routes mount order (3), visitController resolver (8), role helper (7), lookup seed (5), modal (8), service (3), DoctorsPage wiring (9), banner (6). Existing system health check section 11 (`scripts/check-system-health.js`) also extended with a 10-file ten-needle contract verifier — clean. **Vite build**: ✓ 1m 59s, no warnings. **Files added/modified (8)**: 1 NEW backend (`backend/scripts/healthcheckPhaseA5_3_6.js`) + 1 NEW frontend (`frontend/src/components/admin/DuplicateVipClientModal.jsx`) + 6 modified (`backend/controllers/doctorController.js`, `backend/controllers/visitController.js`, `backend/routes/doctorRoutes.js`, `frontend/src/services/doctorService.js`, `frontend/src/pages/admin/DoctorsPage.jsx`, `frontend/src/components/common/PageGuide.jsx`, `scripts/check-system-health.js`). Plus this CLAUDE.md note + a brief CLAUDE-ERP.md cross-ref. **Cross-controller integrity check**: (a) BDM's offline-queued visit replay path in [sw.js](frontend/public/sw.js) hits `/api/visits/` POST → `visitController.createVisit` which now resolves merged-loser refs first; (b) the existing `unique({doctor, user, yearWeekKey})` Visit index keeps weekly-cap enforcement correct *post-redirect* — if a BDM had already logged against the winner this week, the redirect maps the offline replay to the same winner and the unique index correctly rejects as duplicate (existing 11000 catch returns 400 "already been logged this week"); (c) the global E11000 fallback in [errorHandler.js](backend/middleware/errorHandler.js) is unchanged — operates as defense-in-depth for callers that bypass `doctorController` (e.g. legacy curl scripts, future BDM-side editors); (d) regression-protected: the schedule-slot collision branch in createDoctor (txn fallback) still distinguishes canonical-name (return 409 modal) from slot-collision (return 409 generic) via the same `isVipClientNameCleanDuplicate` predicate — verified by healthcheck assertion `"createDoctor txn fallback also calls buildDuplicateVipClient409"`. **Out of scope** (deferred): (a) BDM-side editors hitting the DUPLICATE_VIP_CLIENT 409 still surface as toasts (no modal — BDMs typically can't create Doctors anyway, only edit; rename-collision via PUT is rare and admin-flagged); (b) `EmployeeDashboard` BDM "log offline visit" UX doesn't show the `merge_redirected` toast yet — soft enhancement; (c) approval-mode "approve from inbox" inline button (admin currently grants by opening /admin/doctors → edit chip picker → save, which is fine for MVP). Plan: `~/.claude/plans/phase-a5-canonical-vip-client.md` updated.
