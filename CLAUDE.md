...
13b. **Phase N+ — CLM-finalization gate + always-render CLM panel + bidirectional merged flow** (May 03 2026 v1 + May 04 2026 v2 hardening, both shipped + UI-ratified): closes the merged-encounter UX. v1 (May 03, committed `e61ea49`) shipped the always-render panel + clm_pending gate + bidirectional merged flow. v2 (May 04) hardens it:
    ...
    - **Open follow-ups**: (a) PartnershipCLM doesn't yet read `res.resumed === true` to show a "Resuming your in-progress session" toast — cosmetic UX polish. (b) Photo persisted to IndexedDB under `draftId=X` doesn't restore in the live UI when VisitLogger remounts — CameraCapture writes but doesn't read on mount. Cosmetic (BDM re-uploads). (c) Healthcheck `backend/scripts/healthcheckClmIdempotency.js` not yet created — would assert pre-check + E11000 catch both implement the 200/409 split + frontend passes `X-Idempotency-Key`. (d) **Slide-stats per BDM** is a separate Phase 2 ask — see deferred handoff `memory/handoff_phase_clm_finalization_gate_may04_2026.md` for the plan.
