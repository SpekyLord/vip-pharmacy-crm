...
const pageStyles = `
  ...
  .agd-badge-running { background: #dbeafe; color: #1d4ed8; }
...
function StatusBadge({ status }) {
  const config = {
    success: { icon: CheckCircle, css: 'success' },
    error: { icon: XCircle, css: 'error' },
    partial: { icon: AlertTriangle, css: 'partial' },
    running: { icon: Clock, css: 'running' },
  };
  const current = config[status] || config.success;
  const Icon = current.icon;
  return <span className={`agd-badge agd-badge-${current.css}`}><Icon />{status}</span>;
}
...
export default function AgentDashboard() {
  ...
  const hasRunningAgents =
    startingAgents.length > 0 ||
    runs.some((run) => run.status === 'running') ||
    stats?.agents?.some((agent) => agent.last_status === 'running');
  ...
  const getAgentStatus = (agentKey) => {
    const stat = stats?.agents?.find((agent) => agent._id === agentKey);
    const recentRun = runs.find((run) => run.agent_key === agentKey);
    if (startingAgents.includes(agentKey)) return 'running';
    return stat?.last_status || recentRun?.status || null;
  };

  const isAgentRunning = (agentKey) => getAgentStatus(agentKey) === 'running';
  ...
  const filteredMsgs = msgTab === 'all' ? messages : messages.filter((message) => message.category === msgTab);

  const renderAgentCard = ([key, cfg]) => {
    const Icon = cfg.icon;
    const agent = stats?.agents?.find((entry) => entry._id === key);
          ...
          <div style={{ display: 'flex', gap: 4, alignItems: 'center' }}>
            <span
              style={{
                fontSize: 9,
                fontWeight: 700,
                color: cfg.type === 'AI' ? '#6366f1' : '#10b981',
                background: cfg.type === 'AI' ? '#e0e7ff' : '#dcfce7',
                padding: '1px 6px',
                borderRadius: 4,
              }}
            >
            ...
            {status ? <StatusBadge status={status} /> : <span className="agd-badge" style={{ background: '#f1f5f9', color: '#64748b' }}>Awaiting run</span>}
          </div>
        </div>

        <div className="agd-card-stats">
          <div><div className="agd-stat-value">{agent?.total_runs || 0}</div><div className="agd-stat-label">Total Runs</div></div>
          <div><div className="agd-stat-value">{agent?.total_messages || 0}</div><div className="agd-stat-label">Messages Sent</div></div>
          <div><div className="agd-stat-value">{agent?.total_alerts || 0}</div><div className="agd-stat-label">Alerts</div></div>
          <div><div className="agd-stat-value" style={{ fontSize: 13 }}>{fmtDate(agent?.last_run)}</div><div className="agd-stat-label">Last Run</div></div>
        </div>

        {agent?.last_summary?.key_findings?.length > 0 && (
          <div className="agd-findings" style={{ marginTop: 10 }}>
            {agent.last_summary.key_findings.slice(0, 2).map((finding, index) => (
              <div key={index} className="agd-finding">{finding}</div>
            ))}
          </div>
        )}

        {status === 'error' && agent?.last_error_msg && (
          <div
            style={{
              marginTop: 10,
              fontSize: 11,
          ...
          {isPresidentOrAdmin && (
            <button
              ...
              style={{
                ...
                color: running ? '#1d4ed8' : cfg.color,
    ...
    <div className="agd-page">
      ...
      <div style={{ display: 'flex', flex: 1 }}>
        ...
        <main className="agd-main">
          ...
          {loading ? (
            <div className="agd-loading">Loading agent data...</div>
          ) : (
            <>
              ...
              <div className="agd-cards">
                {Object.entries(AGENT_CONFIG).filter(([, cfg]) => cfg.type === 'AI').map(renderAgentCard)}
              </div>
              ...
              <div className="agd-cards">
                {Object.entries(AGENT_CONFIG).filter(([, cfg]) => cfg.type === 'Free').map(renderAgentCard)}
              </div>
                ...
                {runs.length === 0 ? (
                  ...
                  <table className="agd-table">
                    ...
                    <tbody>
                      {runs.map((run) => (
                        <tr key={run._id}>
                          <td style={{ fontWeight: 600 }}>{run.agent_label}</td>
                          <td>{fmtDate(run.run_date)}</td>
                          <td><StatusBadge status={run.status} /></td>
                          <td>{run.summary?.bdms_processed || 0}</td>
                          <td>{run.summary?.messages_sent || 0}</td>
                          <td>{run.summary?.alerts_generated || 0}</td>
                          <td>
                            <div className="agd-findings">
                              {(run.summary?.key_findings || []).slice(0, 2).map((finding, index) => (
                                <div key={index} className="agd-finding">{finding}</div>
                              ))}
                              {run.error_msg && <div style={{ color: '#dc2626', fontSize: 11 }}>{run.error_msg}</div>}
                            </div>
                          </td>
                        </tr>
                      ))}
              ...
              <div className="agd-section">
                ...
                <div className="agd-tabs">
                  {[['all', 'All'], ['ai_coaching', 'Coaching'], ['ai_schedule', 'Schedules'], ['ai_alert', 'Alerts']].map(([key, label]) => (
                    <button key={key} className={`agd-tab ${msgTab === key ? 'active' : ''}`} onClick={() => setMsgTab(key)}>
                      {label}
                    </button>
                  ))}
                </div>
                {filteredMsgs.length === 0 ? (
                  ...
                  <div className="agd-msg-feed">
                    {filteredMsgs.slice(0, 10).map((message) => (
                      <div
                        className="agd-msg"
                        key={message._id}
                        style={{ borderLeftColor: message.category === 'ai_alert' ? '#ef4444' : message.category === 'ai_schedule' ? '#10b981' : '#6366f1' }}
                      >
                        <div className="agd-msg-title">
                          <span className={`agd-msg-cat agd-msg-cat-${CAT_CSS[message.category] || 'coaching'}`}>
                            {CAT_LABELS[message.category] || message.category}
                          </span>
                          {message.title}
                        </div>
                        <div className="agd-msg-body">
                          {message.body?.slice(0, 200)}
                          {message.body?.length > 200 ? '...' : ''}
                        </div>
                        <div className="agd-msg-meta">
                          {fmtDate(message.createdAt)} • {message.recipientRole} {message.recipientUserId ? '(targeted)' : '(broadcast)'}
                        </div>
                      </div>
                    ))}
                  </div>
