...
const WORKFLOW_GUIDES = {
  ...
  'inbox': {
    ...
    steps: [
      ...
      'Executive Brief folder (Phase G9.R11) = your daily-read signal: morning brief, FP&A forecast, procurement scorecard, expansion readiness. Split out from AI Agents so the high-priority briefs aren\'t buried under operational noise.',
      'AI Agents folder = lower-signal findings from rule-based + AI agents: compliance alerts (entity-guard / orphan / SoD), KPI variance, ai_alerts, inventory reorder, proxy SLA, data-quality digests. Use the priority chips above the list to scan High / Important first when you do click in. Hidden from the main Inbox count for president by default (lookup-driven).',
      ...
      'Per-role folder visibility (Phase G9.R9 + R11): your role may have folders hidden from the rail and the Inbox count via the INBOX_HIDDEN_FOLDERS_BY_ROLE lookup. By default, president has APPROVALS hidden (covered by Approval Hub) and AI_AGENT_REPORTS hidden (kept accessible via direct click; just excluded from the main count). Admin can extend (e.g. add a CEO row, hide TASKS for finance) or disable per role via Control Center → Lookup Tables.',
    ],
    next: [
  ...
  'sales-opening-ar': {
    ...
    steps: [
      ...
      'Scan CSI (required): capture the historical invoice photo — OCR auto-fills doc_ref/date/hospital/line items and persists the photo for audit. Validate blocks Opening AR rows with no proof attached (lookup-driven via SALES_SETTINGS.REQUIRE_CSI_PHOTO_OPENING_AR — default on, ANY proof OK: scanned copy OR signed received CSI). Non-pharmacy / service-only subscribers can flip the flag off in Control Center.',
      ...
      'Save Drafts → saved rows move to Opening AR Transactions (this page resets to a blank row for the next CSI). This page is capture-only — there is no Validate or Post button here. Go to the **Opening AR Transactions** tab to Validate (per-row) and Submit (per-row) each CSI. Posting auto-routes through gateApproval (module=SALES). Period lock for the historical month is bypassed because source=OPENING_AR (set automatically when csi_date < live_date).',
      ...
      'If an approver rejects: a red banner with the reason appears on the row in Opening AR Transactions. Click Fix & Resubmit from the rejection banner to land back on this Entry page pre-loaded for editing; save, then re-Validate + Submit from Transactions.',
    ],
    next: [
    ...
    tip: 'Opening AR is for receivables only. Pre-go-live INVENTORY is loaded with the importOpeningStock script (one-shot at cutover). After cutover is complete, your admin can remove the sales.opening_ar sub-permission to hide this page from the sidebar — entries already posted remain in AR aging. Proxy entry (Phase G4.5a): the "Opening AR — record on behalf of" dropdown surfaces only if you hold role ∈ PROXY_ENTRY_ROLES.OPENING_AR (default admin/finance/president) AND sales.opening_ar_proxy ticked. Historical-data cutover clerks are the intended users. Every proxied Opening AR submit is forced through Approval Hub.',
  },
  ...
  'sales-opening-ar-list': {
    ...
    steps: [
      ...
      'Actions per row (lifecycle all lives here — no bulk buttons anywhere): Validate (DRAFT/ERROR → VALID or ERROR), Submit (VALID → POSTED), Re-open (POSTED → DRAFT, reverses the AR journal), Request Deletion (BDM), Approve Deletion (admin w/ accounting.approve_deletion), President Delete (holders of accounting.reverse_posted).',
  ...
  'sales-entry': {
    ...
    steps: [
      ...
      'CSI/Cash Receipt: add one or more line items per CSI — each line has its own product (warehouse-stock filtered), batch/lot (auto-picked by FIFO), quantity, unit price, and an optional Discount % (Phase R2 — BIR-standard trade discount on the face of the invoice; reduces VAT base per RR 16-2005). Use + Line Item to add another product under the same CSI; the row shows a running Total that sums every line after discount. Service Invoice: enter description + total (no line items).',
      ...
      'CSI only (optional data-entry assist): use "Scan CSI" to OCR a blank/unsigned CSI — it auto-fills doc_ref/date/hospital/line items and stores the scan as an optional audit crumb. Photo is not required here — the signed received-CSI is a separate post-delivery artifact (see next step).',
      'After delivery: go to Sales Transactions (list) and use the 📷 Attach CSI button on the POSTED row to upload the signed pink/yellow/duplicate copy returned by the hospital. This is the dunning-grade proof Finance uses for AR follow-up — it does not change posting status. The list column 📷 flips from ⚠️ to ✓ once attached.',
      ...
      'Save Drafts to persist rows. Validation + posting happen **per row, one CSI at a time** — either from the per-row Validate/Post buttons inside this entry grid, or (canonically) from the Sales Transactions list. There is no bulk Validate or bulk Submit — each CSI carries its own FIFO stock snapshot and VAT balance, so batching could mask a single ERROR in a green "all posted" toast.',
      ...
      'Draft CSI overlay (Phase 15.3): any CSI-type row with at least one line item shows a "📄 Draft CSI" action in Sales Transactions. Clicking downloads a mm-precise PDF that overlays customer/date/items/totals onto a pre-printed BIR booklet page. Feed your booklet into the printer; do NOT print the draft onto blank paper as a standalone receipt (it is not BIR-valid). Calibrate once per printer on the My CSI page. Phase R2 — when any line carries a discount, the Less: Discount totals row (and the Total Sales VAT-Inclusive gross-before-discount row) auto-render in the booklet totals block; per-line Amount stays at qty × unit_price so the booklet math reconciles to the eye.',
      '🩺 Storefront cash MD attribution (Phase R-Storefront, May 2026) — when CASH_RECEIPT or SERVICE_INVOICE routes through a Petty Cash Fund, a green info note appears explaining the post-POSTED workflow: save the row, post it, then go to Sales Transactions → click the row → "🩺 Attach MD Rebate / BDM Commission" to attribute MDs after the walk-in is identified. The Phase 2 (May 8 2026) routing engine then writes a RebatePayout(ACCRUING) per MD, a CommissionPayout(ACCRUING) for the BDM, and a DRAFT PRF per MD payee — all atomic with the save. Real-time on POST + period-close sweep keep PRFs in sync. Routing follows the Phase R1 single-flow lock (PrfCalf only — no PAID_DIRECT) for PRC compliance.',
    ],
    next: [
      { label: 'View All Sales', path: '/erp/sales' },
  ...
  'sales-list': {
    ...
    steps: [
      'All CSI lifecycle actions live here and act **one row at a time**. No bulk buttons. Each CSI carries its own FIFO stock snapshot, VAT balance, and CSI-booklet audit — per-row validation/posting prevents a single bad row from blocking a batch, and prevents a successful batch from masking a silent ERROR.',
      'Statuses: DRAFT (editable, not yet validated) → VALID (passed checks, ready to post) → POSTED (finalized, AR created, appears in reports). ERROR rows have validation messages in the detail panel.',
      'Validate (DRAFT/ERROR) — runs stock/VAT/required-field/credit-limit checks for that CSI only. Flips the row to VALID (Submit appears) or ERROR (messages in detail panel).',
      'Submit (VALID) — posts the CSI: creates AR (or petty cash deposit for cash), deducts stock via FIFO, marks the CSI# used in your booklet. Routes through the Approval Hub for users not in MODULE_DEFAULT_ROLES.SALES (lookup-driven, per-entity). Contractors/BDMs get a "Pending approval" toast.',
      'Re-open (POSTED, admin) — reverses stock, consignment, petty cash deposit, and journal entries (SAP Storno). Blocked if CSI is already settled by a POSTED Collection — reopen the collection first.',
      'Req. Delete (POSTED, non-admin) / Approve Delete (admin w/ accounting.approve_deletion) — legacy two-step deletion flow.',
      'President Delete — for the President (or anyone granted accounting.reverse_posted in Access Templates): one-click delete of bad rows. POSTED rows trigger SAP Storno (reversal entries in current period; original kept for audit); DRAFT/VALID/ERROR rows are hard-deleted. All actions logged.',
      '📷 column (dunning readiness): ✓ means the signed received-CSI is attached — Finance can dunn on this row. ⚠️ on a POSTED row means posting is done but the signed CSI from the hospital is still missing. Click 📷 Attach CSI on any DRAFT / VALID / POSTED row to upload the signed pink/yellow/duplicate copy. Skipped for Opening AR (historical proof is captured at entry). **Phase P1.2 Slice 7-extension Round 2A (May 2026)** — a sibling "From Captures" button next to 📷 Attach CSI lets you pull a BDM-captured signed CSI photo straight from the BDM\'s pending Capture Hub queue without re-uploading from gallery. The picker is narrowed to the row\'s own bdm_id and to workflow_type SALES or UNCATEGORIZED so you only see relevant photos. Server still gates by lookup-driven `CAPTURE_LIFECYCLE_ROLES.PROXY_PULL_CAPTURE` — non-eligible clicks 403 cleanly.',
      'Opening AR — pre-live-date entries skip stock deduction (AR only, no COGS). Manage them on the Opening AR Transactions tab (same per-row actions).',
      '🩺 MD Rebate / BDM Commission (Phase R-Storefront, May 2026) — for storefront cash sales (CASH_RECEIPT or SERVICE_INVOICE routed through a petty cash fund), the Collection-side rebate engine never fires. Open the row\'s detail modal — admin/finance/president see an "Attach MD Rebate / BDM Commission" button beneath the Received CSI block. Pick MDs per line item (one rebate % per partner per product line) plus a top-level commission % for the BDM. Editable post-POSTED — once paid, attribution can be added after the fact when the MD is identified. Server enforces sales.proxy_rebate_entry sub-perm + STOREFRONT_REBATE_SCOPE predicate (default: requires petty_cash_fund_id to avoid double-counting against Collection-side rebate). **Phase 2 (May 8 2026) shipped** — saving attribution now writes a RebatePayout(ACCRUING) row per MD/line, a CommissionPayout(ACCRUING) row for the BDM, and a DRAFT PRF per MD payee, all atomic with the save. PRFs land in the Approval Hub for admin to attach honoraria/CME-grant docs and post — payout follows the Phase R1 single-flow lock (PrfCalf only, never PAID_DIRECT, PRC-compliant). Closed-period edits need accounting.reverse_posted to bypass.',
    ],
    next: [
    ...
    tip: 'Post valid sales promptly. Unposted sales do not count in MTD targets or P&L. Posting routes through the Approval Hub for users not in MODULE_DEFAULT_ROLES.SALES (lookup-driven, per-entity). When Authority Matrix is enabled, additional escalation rules can layer on top. Use the Source filter to view Opening AR entries separately. To re-open a POSTED CSI that has already been collected, first reopen the Collection that settles it — only unsettled CSIs are reopenable. Stuck on an ERROR row? The President can delete + reverse it from this list. A small "Proxied" pill next to the CSI # means the row was keyed by someone other than the owner BDM (Phase G4.5a proxy entry) — tooltip shows who keyed vs who owns. **Phase P1.2 Slice 7-extension Round 2A (May 2026)**: the per-row "From Captures" button next to 📷 Attach CSI is the proxy-friendly path for the signed/pink/duplicate CSI. BDM snaps the signed copy via Quick Capture on /erp/capture-hub (zero typing); the office opens this list, finds the matching CSI #, clicks "From Captures", picks the photo, and the modal auto-uploads it as the dunning proof. No new schema, no re-typing — the picker just bypasses the gallery step.',
  },
  'my-stock': {
  ...
  'smer': {
    ...
    tip: 'SMER is submitted per cycle (every ~15 days), not daily. You can save your SMER as DRAFT and update it daily — submit when the cycle ends. Per diem override: click [+] beside any day to request FULL or HALF tier with a reason. Overrides go to the Approval Hub immediately (even while SMER is DRAFT). Once approved, the override applies automatically and the day turns purple; once rejected, it turns red. Per diem thresholds are per-person (CompProfile). "NO_WORK" days do NOT count as working days and cannot have overrides. **SMER cannot be validated or submitted while any day has a PENDING override** — resolve it in the Approval Hub first (or have the approver decide) so the override amount lands in the journal correctly. **Phase G1.5 (Apr 2026):** Per-diem rate now reads from the PERDIEM_RATES lookup (per-entity × per-role, Control Center → Lookup Tables). No PERDIEM_RATES.<role> row = SMER create blocks with a clear error — seed it first. Visits with flagged photos (duplicate/date-mismatch) are automatically excluded from per-diem when `skip_flagged=true` (default). Weekend per-diem is controlled by `allow_weekend` (default off for pharma). The "locations" note now shows structured `City/Municipality, Province` from the Doctor record — pre-G1.5 legacy doctors fall back to free-text clinic address until the backfill (`backfillDoctorLocality.js`) runs. **Phase G4.5ee (Apr 30 2026) — Activity-aware per-diem.** The activity_type column now drives the per-diem tier directly via the lookup-driven ACTIVITY_PERDIEM_RULES. Defaults: OFFICE → AUTO_FULL (admin/office staff get full ₱650 regardless of MD count), FIELD → USE_THRESHOLDS (existing 8/3 logic), NO_WORK → ZERO. Closes the prior gap where office staff padded MD=10 to clear the 8-threshold. Admin can flip the rule per entity via Control Center → Lookup Tables → ACTIVITY_PERDIEM_RULES. Override paths (admin force-FULL/HALF) intentionally bypass the activity rule — manual override still always wins. **Proxy Entry (Phase G4.5f, Apr 23 2026)** — admin/finance/president (or an office-based eBDM contractor with `expenses.smer_proxy` ticked on their Access Template) can now write a SMER cycle and request per-diem overrides on behalf of another BDM via the BDM picker. The create payload carries `assigned_to` = target BDM so the backend stamps `bdm_id` = target and records `recorded_on_behalf_of` = the proxy. Proxy submits ALWAYS force-route through the Approval Hub (Rule #20 four-eyes), even when the caller has approve rights. Each proxy submit/override requires a short authorization tag (`bdm_phone_instruction`) — short tags like "ok with boss", "in the office", or "with client" are fine; the field is an authorization trail, not a narrative. The target BDM receives a courtesy MessageInbox receipt (PERDIEM_SUMMARY when SMER posts on their behalf, PERDIEM_OVERRIDE_DECISION when the Hub decides their proxied override). Plain BDMs keep `expenses` module access — they can still read their own SMER and log overrides in emergencies — only the proxy WRITE path requires the new sub-perm. Lookup-driven: PROXY_ENTRY_ROLES.SMER (eligible proxy roles) + VALID_OWNER_ROLES.SMER (valid owner roles). Subscribers extend both via Control Center → Lookup Tables. **May 05 2026 — yes-equal-weight pull (Phase G1.7)**: Pull from CRM now sums VIP visits (Doctor) AND EXTRA calls (Client) so the SMER MD count reconciles 1:1 with the CRM list view (MyVisits / EmployeeVisitReport). Pre-fix the bridge silently undercounted on days BDMs logged extras, dropping daily tier from FULL ₱800 → HALF ₱400. Forward-only — already-POSTED SMERs stay frozen; only future Pull-from-CRM clicks and still-DRAFT SMERs pick up the corrected count. Subscribers that want strict VIP-only behavior flip `include_extra_calls=false` on PERDIEM_RATES.<role> via Control Center → Lookup Tables. **Phase G1.7.1 (May 5 2026 evening) — date-column lockstep**: changing the period or cycle dropdowns while a new SMER is being created now auto-regenerates the daily-rows so the row dates always match the heading and the period+cycle the backend will receive. Pre-fix users could click "+ New SMER" for May, switch the dropdowns to April C2, and the rows stayed on May — Pull from CRM then matched by entry_date, found no overlap, and silently left every md_count at 0. The Pull handler also now rebuilds the rows wholesale from the backend response if the row keys still drift apart for any reason (defense in depth — never silently drop counts onto the floor again). **Phase P1.2 Slice 6.1 (May 6 2026 evening) — CRM cities also auto-fill Car Logbook**: the same `City/Municipality, Province` summary that lands in SMER\'s notes column now ALSO populates the Car Logbook destination column directly via a new `pullCrmVisitDestinations` helper — independent of whether the BDM has clicked Pull-from-CRM in SMER yet. The Car Logbook destination cell shows a cyan "CRM" badge when the cities came from this path, so the proxy/admin sees the territory summary the moment they open the grid (no SMER dependency). **Phase SMER-CL (May 07 2026) — CommLog screenshots count toward per-diem**: manual-source `CommunicationLog` rows (Messenger / Viber / WhatsApp / Email / Google Chat) now also count toward the daily MD threshold when admin enables `include_comm_log` on the BDM\'s `PERDIEM_RATES.<role>` row. VIP entity defaults to ON (admin Gregg is in the BDM group chats — fraud-credit is auditable in real-time via Messenger scroll). SaaS subscribers default to OFF (no admin-in-chat trust there). One CommLog row = one MD credit (existing `doctor`/`client` FK). No daily cap by default — admin spot-check is the guard; subscribers can opt into a cap via `comm_log_daily_cap`. Same-day same-MD across Visit + CommLog dedups at merge to 1 (a BDM who visited Dr. X in person AND messaged Dr. X that evening earns 1 MD credit, not 2). Phase O\'s 14-day photo cutoff inherits — chat screenshots older than 14 days at upload time do NOT earn per-diem (`(createdAt - photos[0].capturedAt) ≤ 14d` gate at the bridge). The per-day row shows a blue "💬 N chats" badge when chat-only contributions counted toward the day. Subscribers tune all four knobs via Control Center → Lookup Tables → PERDIEM_RATES.<role>.metadata: `include_comm_log`, `comm_log_daily_cap`, `comm_log_require_outbound`, `comm_log_allowed_sources`.',
  },
  'car-logbook': {
    title: 'Car Logbook',
    steps: [
      'Select period and cycle (C1 = days 1–15, C2 = days 16–end, MONTHLY = full month) — all days including weekends are shown',
      'Phase P1.2 Slice 6 + Slice 6.1 — every day is auto-populated from FIVE sources before you type: CRM Visits → destination as "Iloilo City, Iloilo; Oton, Iloilo" (cyan "CRM" badge — wins over SMER for richest territory signal); SMER daily entry → destination fallback when no CRM visits that day; SMER ODO captures → Start/End KM + photos; DriveAllocation row → Personal/Official KM split; FUEL_ENTRY captures → fuel rows. The colored source badge next to each field tells you where the value came from (CRM, SMER, ODO, Drive, Fuel cap, Prior). Edit any field to override → badge flips to red "Manual"',
      'Override paths still work: tap [S]/[E] to OCR-scan odometer photos manually; type a destination directly; add fuel entries by hand. The auto-populate is purely additive — it pre-fills blanks, never overwrites a saved row',
      'Click the Fuel cell to expand, add fuel entries (station, liters, ₱/L, payment mode) — or tap "Scan Receipt" to OCR a gas receipt directly here',
      ...
      'Click "Save Car Logbook" to persist all dirty rows; an * indicator shows unsaved days. Saved rows lock the auto-populated values — re-running the page won\'t mutate them',
    ...
    tip: '**Phase P1.2 Slice 6 (May 06 2026) + Slice 6.1 (May 06 2026 evening) — Auto-populate**: Car Logbook is now a *review surface* rather than a data-entry form. The page calls `/expenses/car-logbook/preview` per cycle on load, pulling FIVE signals in parallel: **CRM Visits → destination** as `City/Municipality, Province; ...` (Slice 6.1 — yes-equal-weight VIP `Visit` + EXTRA `ClientVisit`, dedupes the day\'s unique localities, wins over SMER fallback because CRM is canonical "where the BDM physically went"; cyan "CRM" badge); SMER `daily_entries.hospital_covered + notes` (destination fallback when no CRM visits exist for the day — typically OFFICE-only days), SMER ODO captures (`workflow_type=SMER` → min reading = Start KM, max = End KM, with photo URLs from each), DriveAllocation row (`status=ALLOCATED` → personal_km / official_km), and FUEL_ENTRY captures (one fuel row per artifact, OCR fields station/fuel_type/liters/₱-per-L). Source badges next to each field show provenance: blue=SMER, green=ODO, amber=Drive (DriveAllocation), pink=Fuel cap (FUEL_ENTRY OCR), indigo=Prior (yesterday\'s ending_km fallback when no SMER ODO today), red=Manual (proxy override). Saving a row (`createCarLogbook?autopopulate=true`) records the per-field provenance map in `edit_history` for audit. The pre-save hook on CarLogbookEntry still owns the math (total_km / official_km / fuel total / efficiency / gas split) — auto-populate writes only the inputs. Subscribers tune via `CAPTURE_LIFECYCLE_ROLES.EDIT_CAR_LOGBOOK_DESTINATION` (default admin/finance/president). Phase 33 cycle-wrapper: submit/post/reverse now run at the cycle level (CarLogbookCycle), not per day. Per-day CarLogbookEntry docs remain the source of truth for odometer/fuel/efficiency — the wrapper only carries cycle-level approval/posting state and aggregated totals. Before Submit, the cycle pre-post gate requires each non-CASH fuel to be either (a) linked to a POSTED CALF or (b) have approval_status=APPROVED via the per-fuel Submit Fuel flow. Rejected fuel entries stay editable; fix the value and Resubmit. Reversing the cycle cascades to all per-day docs in one atomic transaction. COA code for fuel is admin-configurable in Settings (lookup-driven). Privileged viewers (president/admin/finance) use the BDM picker to audit someone else\'s cycle — the page is read-only until they pick themselves (Rule #21 — no silent self-fallback; backend requires an explicit bdm_id to create/validate/submit). **Proxy Entry (Phase G4.5e, Apr 2026)** — admin/finance/president (or an office-based eBDM contractor with `expenses.car_logbook_proxy` ticked on their Access Template) can now WRITE to a selected BDM\'s logbook via the BDM picker. The create payload carries `assigned_to` = target BDM so the backend stamps `bdm_id` = target and records `recorded_on_behalf_of` = the proxy. Any cycle containing a proxy-created day force-routes through the Approval Hub on Submit (Rule #20 four-eyes). Per-fuel Submit Fuel also force-routes when the parent day was proxied. Lookup-driven: PROXY_ENTRY_ROLES.CAR_LOGBOOK (eligible proxy roles) + VALID_OWNER_ROLES.CAR_LOGBOOK (valid owner roles) — subscribers extend both via Control Center → Lookup Tables.',
  },
  'prf-calf': {
    title: 'PRF / CALF',
    steps: [
    ...
    tip: 'Partner rebates follow accrual basis — journal entry (DR 5200 PARTNER_REBATE, CR funding) is created only when PRF is posted, not when the collection is recorded. CALF validates that linked expense line IDs actually belong to the referenced expense entry. Document numbers auto-generate from Territory + date + sequence — if generation fails, check Territory setup for the BDM. All COA codes are admin-configurable in Settings. President Delete — for the President (or anyone granted accounting.reverse_posted in Access Templates): one-click delete of bad PRF/CALF rows. POSTED → SAP Storno (CALF: clears calf_id on non-POSTED expenses; PRF: clears rebate_prf_id on the linked Collection; reverses the associated JE). Blocked if a POSTED downstream doc still depends on it (e.g., POSTED expense funded by this CALF, or an IncomeReport that auto-deducted this CALF — reverse those first). Controller auto-picks CALF vs PRF handler from the row\'s doc_type. All actions logged. **Proxy Entry (Phase G4.5e, Apr 2026)** — admin/finance/president (or an eBDM contractor with `expenses.prf_calf_proxy` ticked) can key a PRF or CALF on behalf of another BDM via the "Record PRF/CALF on behalf of" dropdown on the entry form. When seeded from the pending rebates / pending CALF lines panel the picker auto-defaults to the source doc\'s bdm_id. Any submit containing a proxied doc force-routes through the Approval Hub (Rule #20 four-eyes). Auto-CALF (generated from ACCESS expenses or non-CASH fuel) now propagates `recorded_on_behalf_of` from the source expense/logbook — the proxy audit chain is unbroken across source → CALF → JE. Lookup-driven: PROXY_ENTRY_ROLES.PRF_CALF + VALID_OWNER_ROLES.PRF_CALF. **CALF View Links breakdown (Phase G4.5gg, May 2026)** — clicking View Links on a CALF row expands an inline table with Category, BIR flag, Net-of-VAT, VAT, Amount per linked source, so reviewers can audit BIR reportability and VAT split without leaving the page. Fuel rows derive Net/VAT from the lookup-driven Settings VAT rate (the JE engine resolves the same rate at post time); expense rows surface the values persisted by ExpenseEntry pre-save hook. The same breakdown is shown in the Approval Hub via DocumentDetailPanel.CalfLinkedLines.',
  },
  'collaterals': {
  ...
  'transfers': {
    ...
    steps: [
      ...
      'Same-custodian rebalance (Phase G4.5dd-r2): pick the SAME custodian on both sides to shift your own stock between two of your warehouses. Warehouses must differ. A BDM moving their own stock does not need the proxy grant — the request is self-served.',
      'Phase 32R-Transfer-Stock-Scope (May 07 2026) — Product dropdown is scoped to the Source Custodian\'s stock at the Source Warehouse. The picker stays disabled until BOTH are chosen, so a BDM cannot accidentally move stock they don\'t actually own at the warehouse they picked. Same gate applies to IC and Internal modals; changing the warehouse mid-edit re-fetches and clears the batch cache so cross-warehouse batches don\'t leak.',
      'Subsidiary entities can browse parent company products on IC transfers — configurable via PRODUCT_CATALOG_ACCESS lookup.',
      'Validate and Post (IC) updates both warehouses. Cross-BDM Internal reassignment goes PENDING → APPROVED (FIFO-deducts source) → AWAITING_GRN → COMPLETED (after receiver enters GRN). Same-custodian rebalance closes directly: PENDING → APPROVED → COMPLETED in one step (admin approval writes both TRANSFER_OUT and TRANSFER_IN ledger rows).',
      'Two-person rule on Internal: any user with the create grant can submit a PENDING reassignment, but APPROVE — which deducts FIFO stock from source and shifts ownership — is reserved for admin/finance/president. The two-person rule applies to same-custodian rebalances too.',
    ],
    next: [
      { label: 'Receive Transfer', path: '/erp/transfers/receive' },
      { label: 'View Inventory', path: '/erp/my-stock' },
    ],
    tip: 'Inter-company transfers (between entities) use transfer prices set by the president. Subsidiary product catalog access is lookup-driven. Transfer refs use the source entity\'s short_name as the code (admin-editable in Entity management, cache-busted on rename) — subsidiaries get their own prefix without a code change, matching JE/CALF/PO numbering. Legacy transfers created before the numbering rollout show their original ICT-YYYYMMDD-NNN format; new transfers use the unified format. Phase G4.5dd (Apr 30 2026): an eBDM (back-office BDM) can create Internal reassignments on behalf of field BDMs by ticking inventory.internal_transfer_proxy on their Access Template + appending \'staff\' to PROXY_ENTRY_ROLES.INTERNAL_TRANSFER in Control Center → Lookup Tables. Approval stays admin/finance/president regardless — that gate is non-delegable. Phase 32R (May 07 2026): the product picker enforces (BDM ∩ warehouse) intersection at the FIFO engine — shared warehouses (e.g. ACC — Shared Services) holding multiple BDMs\' stock no longer cross-leak in the dropdown OR at consume time.',
  },
  'transfers-receive': {
  ...
  'myIncome': {
    ...
    steps: [
      ...
      '4. Deduction Schedules: create recurring installment plans or one-time deductions — every submission routes through the Approval Hub (/erp/approvals). Approved installments auto-inject into future payslips. You can edit or withdraw pending schedules from here, and resubmit rejected ones with changes. The Hub\u2019s Approval History tab records every decision.',
  ...
  'income': {
    ...
    steps: [
      ...
      '3. Schedules tab: Review and approve BDM deduction schedules. Filter by status/BDM, create schedules for BDMs, adjust installment amounts, or apply early payoff. BDM-submitted schedules ALSO appear in the Approval Hub (/erp/approvals) — approving from either surface posts the same decision and writes the Approval History row.',
  ...
  'grn-entry': {
    ...
    steps: [
      ...
      'Approve a PENDING GRN: the green Approve button on the row is gated on the linked Undertaking being ACKNOWLEDGED first (Phase 32R — approval flows through the UT, not the GRN). The Undertaking column shows the UT# + status; if it is DRAFT/SUBMITTED, the Approve button is disabled with a hint and an "Open Undertaking →" link that deep-links you straight to the right place. President bypass: presidents can approve directly from the GRN row in emergencies. Reject is always available regardless of UT state — rejecting the GRN does not auto-reject the linked UT.',
    ],
    next: [
  ...
  'approval-manager': {
    ...
    steps: [
      ...
      'CALF Expense Breakdown (May 2026): when you expand a CALF row, the panel now lists the underlying expense lines inline — date, source (FUEL or EXPENSE), category, vendor, BIR filing flag (Internal / BIR / Both), VAT, net, and amount, plus a variance vs the CALF advance. No need to navigate to the Expense module to see what was actually spent.',
      'GRN ↔ Undertaking pairing (Phase 32R-VP, May 6 2026): the GRN card now shows the same proxy badge ("On behalf of <name>") and a structured "Linked Undertaking: UT-XXX · receipt date · scanned/total · variances · status" badge that the UT card already had. If a GRN was proxied, BOTH the GRN and UT cards display the violet proxy pill — they always agree because the UT inherits the field from its parent GRN. The old truncated "_id.slice(-8)" hash chip on the GRN side is gone.',
      'Approval History tab: historical ApprovalRequest decisions (APPROVED / REJECTED / CANCELLED) for Authority-Matrix audit. Filter by status or module. Phase G4.7 (May 2026) — module-native decisions also surface here: every income-report transition (review / return / confirm / credit), finance-auto-created deduction schedule, and admin-cancelled ACTIVE schedule writes a closed audit row via the shared mirrorApprovalDecision helper, so the unified history reflects ALL approval-like decisions, not just the Hub-routed ones.',
  ...
  'people-list': {
    ...
    steps: [
      ...
      'List visibility honors multi-entity span (Phase G7): a person appears in this entity\'s list when this is their home entity OR they have a static grant via Entity Access OR they hold an active Functional Role here',
      'To make someone visible in another entity, open their profile → Entity Access → Grant Entity (or Transfer Home to move them entirely)',
    ],
    next: [
      { label: 'Org Chart', path: '/erp/org-chart' },
      { label: 'Add Person', path: '/erp/people' },
    ],
    tip: 'The Active tab shows current employees (ACTIVE, ON_LEAVE, SUSPENDED). The Archive tab shows separated employees. Use "Sync from CRM" to import existing CRM users (creates them under the currently-selected entity). Phase G7 visibility union: this list now includes spanned people (User.entity_ids) and FRA holders, not just home-entity rows — so a parent-VIP staff granted access to MG and CO. shows up under both. Reactivating a person sets them to ACTIVE but does not restore login or role assignments — those must be re-enabled manually. Cross-entity view (president toggle) is governed by the CROSS_ENTITY_VIEW_ROLES.PEOPLE_MASTER lookup. Note: all people selection dropdowns across the ERP (Managed By, Reports To, Assign To, etc.) only show ACTIVE people.',
  },
  'person-detail': {
    ...
    steps: [
      ...
      'Entity Access (Phase G7): Home Entity is the person\'s primary scope; Additional Entities widen visibility and unlock cross-entity proxy on transactional documents. Use "Transfer Home" to move primary scope; use "+ Grant Entity" to add additional access',
    ],
    next: [
    ...
    tip: 'Separating an employee cascades: marks SEPARATED, revokes all role assignments, and disables system login. Reactivation restores ACTIVE status only — re-enable login and role assignments manually. Career path: CONTRACTOR → PS_ELIGIBLE → TRANSITIONING → SUBSIDIARY → SHAREHOLDER. Position and Department are lookup-driven — to add new options, go to Control Center → Lookup Tables. Entity Access: Transfer Home blocks if there are POSTED documents in the source entity within the configured lookback (default 90 days; per-entity tunable via Lookup PEOPLE_LIFECYCLE_CONFIG.TRANSFER_BLOCK_LOOKBACK_DAYS). Granting an entity widens VISIBILITY only — module-level access still requires an Access Template assignment per entity. Sub-perms people.transfer_entity and people.grant_entity are danger-baseline (require explicit grant on Access Template, even for module-FULL admins).',
  },
  ...
  'csi-booklets': {
    ...
    steps: [
      ...
      'Open a booklet → allocate a small range (typically 3–7 numbers) to a specific BDM. No dates required. The "Assign to BDM" dropdown is sourced from the proxy roster (Phase G4.5gg) so it shows only roles in VALID_OWNER_ROLES.CSI_BOOKLETS — by default that\'s `staff` (BDMs). Admin/finance/president are excluded as assignees because they have no /my-csi self-service tab and the auto-mark-used-on-sale join keys on bdm_id.',
