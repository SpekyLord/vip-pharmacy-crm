...
const WORKFLOW_GUIDES = {
  ...
  'sales-entry': {
    ...
    tip: 'CSI sales always create AR (collect via Collections). Cash Receipt and Service Invoice with CASH payment can route directly to a Petty Cash Fund — bypassing AR and auto-creating a deposit on posting. Only ACTIVE funds with REVOLVING or DEPOSIT_ONLY mode are available. Re-opening a CSI that a Collection already settled is blocked to keep AR and GL balanced — reopen the Collection first. Proxy entry (Phase G4.5a): if the "Record on behalf of" dropdown shows at the top of the form, picking a BDM files the CSI under their bdm_id; every proxied submit is forced through Approval Hub regardless of your role (owner approval required, four-eyes guard). Eligibility is two-layer: role ∈ PROXY_ENTRY_ROLES.SALES (lookup, default admin/finance/president) AND sales.proxy_entry ticked on Access Template. Delegate to a back-office contractor by adding `contractor` to the lookup.',
  },
  'sales-list': {
