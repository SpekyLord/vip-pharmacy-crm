...
const PAGE_GUIDES = {
  ...
  'bir-2307-inbound': {
    ...
    steps: [
      ...
      'Workflow: Hospital pays VIP → Collection POSTED → CwtLedger row created PENDING_2307 → Hospital eventually mails / emails BIR Form 2307 paper or PDF → bookkeeper opens THIS page → finds the matching CR row → clicks "Mark Received" → either pulls the photo from BDM Captures (Slice 7-extension Round 2C — see step 4) OR types where they saved the file (Drive link / S3 URI / shared folder path) + filename + optional SHA-256 hash → row flips to RECEIVED → credit rolls into 1702 (Phase J7).',
      ...
      'NEW (Phase P1.2 Slice 7-extension Round 2C): inside the Mark-Received modal, click "From BDM Captures" to pick a 2307 photo a BDM already uploaded via the Capture Hub (workflow_type = CWT_INBOUND or UNCATEGORIZED / Quick Capture). The picker shows pending captures from ALL BDMs in your entity (cross-BDM scope — finance matches by amount + hospital), writes the bare S3 URL straight into Certificate URL, auto-fills filename, and on Save the source CaptureSubmission flips out of PENDING_PROXY (PROCESSED / back-linked to the CwtLedger row). cert_content_hash + Notes stay manual — hash is computed by you from the actual file bytes; notes are human judgment.',
      ...
      'IMPORTANT: we do NOT store the PDF file bytes. Either the BDM-captured photo lives in S3 (via Capture Hub) and the page records the bare URL, OR the bookkeeper hosts the PDF wherever convenient (Drive / S3 / shared filesystem) and types the URL. The audit trail is the URL + filename + optional SHA-256 hash + capture-back-link (when picker-sourced) + received_at / received_by timestamp on the CwtLedger row. BIR auditors examining the trail can fetch the PDF themselves.',
