/**
 * CommissionMatrixPage — Phase VIP-1.B Phase 4
 *
...
export default function CommissionMatrixPage() {
  ...
  return (
    <div style={{ display: 'flex', minHeight: '100vh', background: '#f8fafc' }}>
      ...
      {showCreate && <CreateCommissionModal payeeRole={tab} users={users} onClose={() => setShowCreate(false)} onCreated={() => { setShowCreate(false); load(); }} />}
    </div>
  );
}

function CreateCommissionModal({ payeeRole, users, onClose, onCreated }) {
    ...
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000 }}>
      <form onSubmit={onSubmit} style={{ background: '#fff', borderRadius: 8, padding: 24, width: '90%', maxWidth: 540 }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr', gap: 12, marginBottom: 12 }}>
          <div>
            <label style={{ display: 'block', fontSize: 12, marginBottom: 4 }}>Product code (optional, exact match)</label>
