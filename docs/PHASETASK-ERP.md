...
**Trigger:** Live SMER review of Judy Mae Patrocinio's 2026-04-C2 cycle showed 11 OFFICE days × MD=10 = 110 phantom MD encounters. Net effect on payroll: zero (with Full Threshold = 0 on her CompProfile, any MD count yielded FULL anyway). Net effect on data integrity: 110 MD visits in DCR Call Rate that don't exist. Fix the design so the workaround becomes unnecessary.

### G4.5ee.1 — Backend resolver + tier rule wiring ✅
- [x] [backend/erp/services/perdiemCalc.js](../backend/erp/services/perdiemCalc.js) — extend `computePerdiemTier(...)` + `computePerdiemAmount(...)` with optional `options.activityRule`. Default `options = {}` keeps every untouched call site byte-identical.
- [x] Add `resolveActivityPerdiemRule(entityId, code)`, `resolveActivityPerdiemRuleMap(entityId)`, `getActivityRuleFromMap(map, code)`, `invalidateActivityPerdiemRuleCache(entityId)` with 60s in-process cache (mirrors PROXY_ENTRY_ROLES / PERDIEM_RATES pattern).
- [x] Inline `ACTIVITY_PERDIEM_RULE_DEFAULTS` (OFFICE → AUTO_FULL, FIELD → USE_THRESHOLDS, OTHER → USE_THRESHOLDS, NO_WORK → ZERO) so resolver works on un-seeded entities.
- [x] `VALID_TIER_RULES` Set (`AUTO_FULL`, `AUTO_HALF`, `ZERO`, `USE_THRESHOLDS`) gates malformed `metadata.tier_rule` defensively.

### G4.5ee.2 — Lookup seed + cache-bust hook ✅
- [x] [backend/erp/controllers/lookupGenericController.js](../backend/erp/controllers/lookupGenericController.js) — add `SEED_DEFAULTS.ACTIVITY_PERDIEM_RULES` with 4 baseline rows (`insert_only_metadata: true` so admin tweaks survive re-seeds).
- [x] Wire `invalidateActivityPerdiemRuleCache` into all 4 mutator paths (create / update / remove / seedCategory) so admin edits propagate immediately.

### G4.5ee.3 — expenseController migration (3 sites) ✅
- [x] `createSmer` — prefetch rule map via `resolveActivityPerdiemRuleMap(req.entityId)`, thread `{ activityRule }` into non-override `computePerdiemAmount` calls.
- [x] `updateSmer` — same prefetch + same option pass-through.
- [x] `remove_override` revert path — resolve activity rule for the reverted entry so OFFICE day returns to AUTO_FULL (not the MD-threshold result the override was masking).
- [x] **Override paths intentionally NOT migrated** — createSmer override branch, updateSmer override branch, override-request, override-apply. They pass fake md_count (999/3) and NO options so admin force-FULL/HALF always wins.
- [x] **CRM-Pull preview path intentionally NOT migrated** — first-load preview before BDM picks activity_type. After Save, createSmer applies the rule.

### G4.5ee.4 — Frontend mirror in Smer.jsx ✅
- [x] [frontend/src/erp/pages/Smer.jsx](../frontend/src/erp/pages/Smer.jsx) — `useLookupOptions('ACTIVITY_PERDIEM_RULES')` + `useMemo` rule map with inline fallback (page never goes dark on a Lookup outage).
- [x] `computePerdiem(count, activityType)` — accept activityType, short-circuit AUTO_FULL/AUTO_HALF/ZERO, fall through to MD-threshold for USE_THRESHOLDS/unset.
- [x] Update 2 call sites: `handleEntryChange` (after activity_type or md_count change) + `handleRemoveOverride` (local-state revert fallback).
- [x] **Override-applied path keeps `computePerdiem(tier === 'FULL' ? 999 : 3)`** — no activity_type passed, override always wins.
- [x] Activity-rule banner: renders only when at least one rule is non-USE_THRESHOLDS (pharma-default subscribers see no banner; OFFICE/NO_WORK subscribers see explanatory copy).

### G4.5ee.5 — WorkflowGuide narrative ✅
- [x] [frontend/src/erp/components/WorkflowGuide.jsx](../frontend/src/erp/components/WorkflowGuide.jsx) — extend `WORKFLOW_GUIDES.smer.tip` with the G4.5ee paragraph explaining activity-driven per-diem and the OFFICE → AUTO_FULL default.

### G4.5ee.6 — Healthcheck + regression validation ✅
- [x] [backend/scripts/healthcheckActivityPerdiemRules.js](../backend/scripts/healthcheckActivityPerdiemRules.js) — NEW, 34-check static wiring contract.
- [x] **34/34 PASS** on first run (after one regex hint adjustment).
- [x] Regression healthchecks: SalesDiscount 41/41, SmerRevert 18/18, ComputePayrollProxy 29/29, PayslipProxyRoster 31/31, IncomeProxy 32/32 — all green.
- [x] Vite full bundle green (28.25s, no chunk-size regressions).
- [x] Backend syntax check (`node -c`) clean on all 3 modified backend files.
- [x] Live Playwright smoke as `s25.vippharmacy@gmail.com` (Judy Mae Patrocinio) on `/erp/smer` — cycle 2026-05-C1 — verify OFFICE day shows FULL ₱650 with MD=0; FIELD day shows ZERO at MD=0, FULL at MD≥8; banner renders correctly.

### Files touched (8 code + 1 healthcheck + 2 docs = 11)
- `backend/erp/services/perdiemCalc.js` (resolver + cache + tier/amount + 4 new exports)
- `backend/erp/controllers/lookupGenericController.js` (seed + cache-bust hook on 4 paths)
- `backend/erp/controllers/expenseController.js` (3 migration sites, 7 override sites untouched)
- `frontend/src/erp/pages/Smer.jsx` (rule map hook + computePerdiem + banner)
- `frontend/src/erp/components/WorkflowGuide.jsx` (smer.tip extended)
- `backend/scripts/healthcheckActivityPerdiemRules.js` (NEW)
- `CLAUDE-ERP.md` + `docs/PHASETASK-ERP.md` (this section + the CLAUDE-ERP narrative)

### Subscription readiness
Lookup-driven per Rule #3 / Rule #19. Subscribers extend ACTIVITY_PERDIEM_RULES via Control Center → Lookup Tables. Cache + buster are per-entity; no cross-tenant leak risk at SaaS spin-out (Rule #0d). Future delivery-driver SaaS ships with `OFFICE → ZERO`, `FIELD → AUTO_FULL`. Future short-term-rental SaaS ships with `INSPECTION → AUTO_FULL`, `OFFICE → USE_THRESHOLDS`.
...
### UX-Scroll.6 — Followup hygiene sweep (remaining 56 ERP pages) ✅ COMPLETE (Apr 30, 2026 evening, same session)

Swept the remaining `<page>-main { overflow-y: auto }` rules across the rest of the ERP page tree via atomic Node.js script (`c:/tmp/sweep_overflow.mjs`). Each replacement verified one-match-only; zero ambiguous matches; zero misses.

**56 files cleaned (single mechanical pass, all OK):**
`AccountsPayable` (`.ap-main`), `AgentDashboard` (`.agd-main`), `AuditLogs` (`.audit-main`), `BankAccounts` (`.ba-main`), `BankReconciliation` (`.br-main`), `BirCalculator` (`.bir-main`), `CashflowStatement` (`.cf-main`), `ChartOfAccounts` (`.coa-main`), `ConsignmentAging` (`.aging-main`), `CostCenters` (`.cc-main`), `CreditCardLedger` (`.ccl-main`), `CreditCardManager` (`.ccm-main`), `CreditRuleManager` (`.crm-main`), `CsiBooklets` (`.booklet-main`), `CycleReports` (`.cr-main`), `CycleStatusDashboard` (`.cycle-main`), `DataArchive` (`.archive-main`), `DisputeCenter` (`.dsp-main`), `ExpenseAnomalies` (`.anomaly-main`), `FixedAssets` (`.fa-main`), `FuelEfficiency` (`.fuel-main`), `GovernmentRates` (`.govr-main`), `Income` (`.income-main`), `IncentivePayoutLedger` (`.ipl-main`), `IncentiveTracker` (`.ict-main`), `JournalEntries` (`.je-main`), `KpiTemplateManager` (`.ktm-main`), `Loans` (`.ln-main`), `MonthEndClose` (`.mec-main`), `MonthlyArchive` (`.archive-main`), `MyIncome` (`.my-income-main`), `OpeningArEntry` (`.oar-main`), `OwnerEquity` (`.oe-main`), `PaymentModes` (`.pmode-main`), `PerformanceRanking` (`.perf-main`), `PeriodLocks` (`.plk-main`), `Pnl` (`.pnl-main`), `ProductMaster` (`.pm-main` — also dropped paired `-webkit-overflow-scrolling: touch`), `ProfitAndLoss` (`.pl-main`), `ProfitSharing` (`.ps-main`), `PurchaseOrders` (`.po-main`), `RecurringJournals` (`.rj-main`), `RxCorrelation` (`.rx-main`), `SalesGoalBdmView` (`.bdv-main`), `SalesGoalDashboard` (`.sgd-main`), `SalesGoalSetup` (`.sgs-main`), `ScenarioPlanner` (`.sp-main`), `SoxControlMatrix` (`.sox-main`), `SupplierInvoices` (`.si-main`), `TasksPage` (`.tsk-main`), `TransferPriceManager` (`.tpm-main`), `TransferReceipt` (`.receipt-main`), `TrialBalance` (`.tb-main`), `VarianceAlertCenter` (`.va-main`), `VatCompliance` (`.vat-main`), `VendorList` (`.vl-main`).

- [x] Atomic Node script (`c:/tmp/sweep_overflow.mjs`) applied 56 single-match replacements; 1 file already-clean from UX-Scroll.2 (IcSettlement, redundantly listed in script — correctly skipped).
- [x] Post-sweep grep `^\s*\.[a-z][a-z-]*-main\s*\{[^}]*overflow-y:\s*auto` returns **zero matches** across `frontend/src/erp/pages/` — anti-pattern fully eliminated.
- [x] Vite build green (10.60s, no chunk-size or syntax regressions).
- [x] OcrTest opt-in untouched ([OcrTest.jsx:46-55](../frontend/src/erp/pages/OcrTest.jsx#L46-L55)) — its scoped `.admin-page.ocr-page .admin-main { overflow-y: auto }` (with the `.admin-page.ocr-page { height: 100vh; overflow: hidden }` height-cap re-establishment) is preserved exactly.
- [x] Post-sweep Playwright smoke on `/erp/accounts-payable` as president: `.ap-main` `getComputedStyle().overflowY` reports `"visible"` (was `"auto"` pre-sweep — rule successfully removed); body still scrolls past viewport (`bodyScrollHeight: 3546`, viewport `774`); WorkflowGuide visible; `bodyScrollMechanicsWork: true`. User-facing behavior identical to pre-sweep, CSS now clean.

**Total Phase UX-Scroll surface (UX-Scroll.1 + UX-Scroll.2 + UX-Scroll.6):** 1 root (`index.css`) + 30 hygiene Round 1 (UX-Scroll.2) + 56 hygiene Round 2 (UX-Scroll.6) + 2 docs = **89 files touched**. Anti-pattern eliminated across ~86 ERP pages. Zero remaining `*-main { overflow-y: auto }` rules in `frontend/src/erp/pages/` (verified by post-sweep grep).
...
- [x] **🔴 Critical bug fix — `SalesEntry.createSale` payload silently dropped `line_discount_percent`.** [frontend/src/erp/pages/SalesEntry.jsx:887-895](../frontend/src/erp/pages/SalesEntry.jsx#L887) `validItems.map()` did not include `line_discount_percent` in the payload. The form rendered the input, `computeLineTotal` showed the correct discounted total client-side, but the API call DROPPED the field → backend persisted with default 0 → page reload showed full gross. **R2 was silently broken end-to-end despite the original 33/33 healthcheck PASS.** Caught only by browser smoke as Mae Navarro (staff role, non-privileged): typed disc=15 → ₱4,080 client total → Save → row persisted as ₱4,800. **FIX:** added `line_discount_percent: Math.max(0, Math.min(100, parseFloat(li.line_discount_percent) || 0))` to the payload map. Healthcheck assertion #41 (`SalesEntry createSale payload includes line_discount_percent`) blocks regression of this specific class of "form field rendered but not in API call" bug for all future R-phase line-level fields.
- [x] **Healthcheck extended** — added 8 assertions (now 41/41 PASS, was 33/33).
- [x] **Live Playwright UI smoke (deferred from original ship — closed Apr 30 2026 evening)** — full happy path ratified end-to-end as Mae Navarro / s3.vippharmacy@gmail.com (staff). `/erp/sales/entry` → CSI #8889 / Viptriaxone 1g × 10 @ ₱480 with Disc%=15 → live total ₱4,080 → Save Drafts (post-fix; prior CSI #8888 reproduced the bug pre-fix) → Validate → VALID → Submit → HTTP 202 routed to Approval Hub (Mae's staff role not in `MODULE_DEFAULT_ROLES.SALES`) → re-login as president (`/erp/approvals`) → Details panel rendered new **Gross (₱4,800.00)** + **Disc % (15%)** columns + footer **Total Sales (VAT Inclusive): ₱4,800.00 → Less: Discount: (₱720.00)** in amber → Net of VAT ₱3,642.86 → VAT ₱437.14 → Total ₱4,080.00. **BIR math reconciles**: 4800 − 15% = 4080; VAT base shrinks (4080/1.12 = 3642.86); VAT = 437.14; AR receivable = 4080. Both smoke fixtures President-Deleted post-verification with audit reasons recorded. Screenshots: `r2-discount-math-smoke.png`, `r2-mae-discounted-row.png`, `r2-mae-after-save.png` (bug-repro), `r2-mae-after-fix.png`, `r2-mae-fixed-row-detail.png`, `r2-approval-hub-with-sales.png`, `r2-approval-hub-sales-detail.png` (definitive).
- [x] CLAUDE-ERP.md updated with Apr-30-evening hardening sub-section under Phase R2.

**Files touched (post-original-ship hardening, 5):**
- `frontend/src/erp/pages/SalesEntry.jsx` — **🔴 critical bug fix** — `line_discount_percent` in createSale payload (line 897)
...
- `backend/scripts/healthcheckSalesDiscount.js` — 8 new assertions (33 → 41)

**Lesson (codified for future R-phases):** A green static healthcheck is necessary but not sufficient. New form-field → API-payload mappings need explicit "field is IN the network call" assertions, not just "field is rendered + computed client-side." For any UI/business-logic surface, drive the actual happy path in Playwright (or equivalent browser smoke) before declaring done — the iron law from global Rule #0b.
...
> Tightly-scoped regression fix. Closes the "Petty Cash cannot be approved from the Approval Hub" 500-error.

### Symptom

President clicks **Approve** on any PETTY_CASH item in /erp/approvals → DevTools shows `POST /api/erp/approvals/universal-approve 500 (Internal Server Error)` → toast "Something went wrong". Reject worked; approve didn't.

### Root cause

The Group B `petty_cash` handler at universalApprovalController.js L980-985 was a bare delegate to `buildGroupBReject`, which throws on any action ≠ `'reject'` (handler L1020-1022). The original Phase G6.7 design said the approve path was "consumed by each module's controller on next call via isFullyApproved()" — but in practice the Hub never flipped the `ApprovalRequest` to APPROVED (handler threw before the auto-resolve at L1130-1160 ran), so the two-step never closed the loop. Net effect: Hub could **only reject** petty cash; approves were dead.

### Tasks

- [x] Extract `postSinglePettyCashTransaction(txnId, userId)` helper from `postTransaction` (pettyCashController.js L281-378)
  - [x] Idempotent on POSTED (early return with `already_posted: true`)
  - [x] Period lock against `precheck.entity_id` (cross-entity-safe for privileged approvers)
  - [x] Atomic balance + status flip in `session.withTransaction()`
  - [x] Insufficient-balance guard on DISBURSEMENT (statusCode-tagged 400)
  - [x] Stamps `posted_by` / `posted_at` / `approved_by` (when blank) / `running_balance`
  - [x] Best-effort post-commit ceiling-breach notify (custodian + president)
- [x] Refactor `postTransaction` to call the helper after `gateApproval`
- [x] Wire `approvalHandlers.petty_cash` branches on action: reject → `buildGroupBReject`; approve | post → deref ApprovalRequest → helper → close request to APPROVED with history $push
- [x] Healthcheck `node backend/scripts/healthcheckPettyCashHubApprove.js` — 22/22 PASS
- [x] `node -c` on both modified controllers — green
- [x] `npx vite build` — green in 13.96s
- [x] Regression: G4.5dd 26/26, Income Proxy 32/32, Compute Payroll 29/29, Payslip Roster 31/31 — all green
- [ ] Live Playwright smoke (s25 BDM submits → president approves → POSTED + balance + request closed) — see handoff

### Files touched

- modified: backend/erp/controllers/pettyCashController.js
- modified: backend/erp/controllers/universalApprovalController.js
- new: backend/scripts/healthcheckPettyCashHubApprove.js
...
| incentive_payout| ✅ G6.7-PC3 | IncentivePayout                   | postSinglePayoutApproval / postSinglePayoutPayment / postSinglePayoutReversal (NEW × 3) — branches on doc_type |
| purchasing      | ✅ G6.7-PC4 | SupplierInvoice                   | postSingleSupplierInvoice            |
| sales_goal_plan | ✅ G6.7-PC5 | SalesGoalPlan                     | postSingleSalesGoalPlan (7-cascade activation) — branches on doc_type |
| ic_transfer     | ✅ G6.7-PC6 | InterCompanyTransfer / IcSettlement | approveSingleIcTransfer + postSingleIcSettlement — branches on doc_type |
| banking         | ✅ G6.7-PC7 | BankStatement                     | finalizeRecon (service-layer, with idempotent status peek in Hub) |

**Group B Hub Approve regression class — CLOSED.** All seven handlers branch correctly on action and dispatch through extracted lifecycle helpers. SaaS spin-out audit gate cleared on this surface. (190/190 assertions across PC1-PC7 healthchecks PASS.)
...
> Same fix template as G6.7-PC1+PC2 with one extra layer: the `incentive_payout` Hub handler must branch on `ApprovalRequest.doc_type` because the BDM-direct controller emits FOUR distinct doc_types under one module key (`PAYOUT_APPROVE`, `PAYOUT_PAY`, `PAYOUT_REVERSE`, `STATEMENT_DISPATCH`). Each maps to a different lifecycle helper.

### Symptom

President clicks **Approve** on any INCENTIVE_PAYOUT item in /erp/approvals → `POST /api/erp/approvals/universal-approve 500 (Internal Server Error)`. Same bug class as petty_cash + journal; same fix pattern with doc_type dispatching.

### Root cause

`approvalHandlers.incentive_payout` at universalApprovalController.js was a bare `buildGroupBReject` delegate. `buildGroupBReject` throws "Unsupported action for incentive_payout: approve — only 'reject' is supported via Group B handler" on any action ≠ `'reject'`.

### Tasks

- [x] Extract `postSinglePayoutApproval(payoutId, userId, notes?)` from `approvePayout` route — `ACCRUED → APPROVED` (no JE, no period lock — pure status flip + audit + integration event)
- [x] Extract `postSinglePayoutPayment(payoutId, userId, paidViaOpts?)` from `payPayout` route — `ACCRUED|APPROVED → PAID` with settlement JE, period-locks against payout's entity, falls back to CASH_ON_HAND for funding COA when paid_via blank
- [x] Extract `postSinglePayoutReversal(payoutId, userId, reason)` from `reversePayout` route — Any non-REVERSED → `REVERSED` with SAP Storno via existing `reverseAccrualJournal`
- [x] All three helpers idempotent on terminal state — re-firing from Hub never double-posts
- [x] All three exported from `incentivePayoutController.js`
- [x] BDM-direct routes (`approvePayout` / `payPayout` / `reversePayout`) refactored to delegate to the helpers — single source of truth for state transitions
- [x] Branch `approvalHandlers.incentive_payout` on action: reject → `buildGroupBReject` (unchanged with widened `modelByDocType` map covering all 4 doc_types); approve | post → deref ApprovalRequest → branch on `request.doc_type` (PAYOUT_APPROVE / PAYOUT_PAY / PAYOUT_REVERSE / STATEMENT_DISPATCH) → call appropriate helper → close request
- [x] STATEMENT_DISPATCH short-circuits with no payout state change (request.doc_id points at requester userId, not a payout); just close the request — admin re-runs `/statements/dispatch` to actually mass-mail
- [x] Healthcheck `node backend/scripts/healthcheckIncentivePayoutHubApprove.js` — **34/34 PASS**
- [x] Regression: G6.7-PC1 22/22, G6.7-PC2 19/19 — both green
- [x] `node -c` on universalApprovalController + incentivePayoutController — both green
- [x] `npx vite build` — green in 16.80s
- [x] Live UI ratify (President at `/erp/approvals`): Hub renders 7 pending items cleanly with "Authority Matrix Disabled" badge + full PageGuide banner — no regression from PC3 patch. Zero pending INCENTIVE_PAYOUT items in dev (no payout has been routed to the Hub yet — Sales Goal cycles haven't been computed). Screenshot: `g6-7-pc3-hub-renders-clean.png`.
- [ ] Live end-to-end Approve smoke deferred — seeding a real IncentivePayout requires SalesGoalPlan + ACTIVE targets + KPI snapshot + accrual JE (heavy fixture). Will fall out of next real Sales Goal cycle. Synthetic-payout fixture documented but not seeded.

### Files touched

- modified: backend/erp/controllers/incentivePayoutController.js — extracted three lifecycle helpers; BDM-direct routes delegate to them
- modified: backend/erp/controllers/universalApprovalController.js — `incentive_payout` handler branches on action AND on `request.doc_type`
- new: backend/scripts/healthcheckIncentivePayoutHubApprove.js
- modified: CLAUDE-ERP.md
- modified: docs/PHASETASK-ERP.md

### Subscription posture

Approver gate still routes through `MODULE_DEFAULT_ROLES.INCENTIVE_PAYOUT` + `approve_incentive_payout` sub-permission. No new lookup category. Same backward-compat guard as PC1+PC2: handler falls back to treating `id` as the payout `_id` directly when `ApprovalRequest.findById(id)` returns null — direct dispatches outside the Hub keep working.

---

## Phase G6.7-PC4 — Purchasing (Supplier Invoice) Hub Approve Fix (May 01 2026)

> Same fix template as PC1+PC2+PC3, applied to `purchasing`. Helper `postSingleSupplierInvoice` was already extracted in prep work; PC4 wires the Hub handler to it.

### Symptom

President clicks **Approve** on any SUPPLIER_INVOICE in /erp/approvals → `POST /api/erp/approvals/universal-approve 500 (Internal Server Error)`. Same bug class as petty_cash + journal + incentive_payout.

### Root cause

`approvalHandlers.purchasing` at universalApprovalController.js was a bare `buildGroupBReject` delegate. `buildGroupBReject` throws "Unsupported action for purchasing: approve — only 'reject' is supported via Group B handler" on any action ≠ `'reject'`.

### Tasks

- [x] Helper `postSingleSupplierInvoice(invoiceId, userId)` extracted from `postInvoice` route (prep work prior to PC4) — single source of truth for `DRAFT|VALIDATED → POSTED`. Idempotent on POSTED, period-locks against invoice's entity, transactional JE post (rollback-safe), INPUT VAT entry post-commit best-effort.
- [x] BDM-direct `postInvoice` route delegates to the helper.
- [x] Branch `approvalHandlers.purchasing` on action: reject → `buildGroupBReject` (unchanged); approve | post → deref ApprovalRequest → call `postSingleSupplierInvoice` → close request to APPROVED with history $push.
- [x] Hub handler today only dispatches `SUPPLIER_INVOICE` doc_type (PO doc_types route through `purchasingController.approvePO` / `receivePO` and never reach this handler). Future PO Hub flows would branch on doc_type here.
- [x] Healthcheck `node backend/scripts/healthcheckPurchasingHubApprove.js` — **25/25 PASS**
- [x] `node -c` on universalApprovalController + purchasingController — both green
- [x] `npx vite build` — green in 35.70s
- [ ] Live UI ratify deferred to bundled smoke at the end of PC3-PC7 sweep (Hub session expired between PC3 ratify and PC4 due to backend hot-reload).

### Files touched

- modified: backend/erp/controllers/universalApprovalController.js — `purchasing` handler branches on action
- new: backend/scripts/healthcheckPurchasingHubApprove.js
- modified: CLAUDE-ERP.md
- modified: docs/PHASETASK-ERP.md

### Subscription posture

Approver gate still routes through `MODULE_DEFAULT_ROLES.PURCHASING` + `approve_purchasing` sub-permission. No new lookup category. Backward-compat guard preserved.

---

## Phase G6.7-PC5 — Sales Goal Plan Hub Approve Fix (May 01 2026)

> Same fix template as PC1-PC4, applied to `sales_goal_plan`. Most cascade-heavy phase — DRAFT → ACTIVE fires SEVEN downstream cascades inside one transaction. Helper `postSingleSalesGoalPlan` was already extracted in prep work; PC5 wires the Hub handler to it.

### Symptom

President clicks **Approve** on any SALES_GOAL_PLAN PLAN_ACTIVATE in /erp/approvals → `POST /api/erp/approvals/universal-approve 500 (Internal Server Error)`. Same bug class as PC1-PC4.

### Tasks

- [x] Helper `postSingleSalesGoalPlan(planId, userId)` extracted from `activatePlan` route (prep work prior to PC5) — single source of truth for the 7-cascade activation: (1) plan.reference assignment if blank, (2) status DRAFT → ACTIVE, (3) SalesGoalTargets DRAFT → ACTIVE, (4) `autoEnrollEligibleBdms`, (5) `ensureKpiVarianceGlobalThreshold`, (6) `incentivePlanService.syncHeaderOnActivation`, (7) STATUS_CHANGE audit. All inside one withTransaction so plan.reference is never burned on rollback.
- [x] Helper idempotent on plan.status === 'ACTIVE' — re-clicking Approve from Hub never double-enrolls or burns a fresh reference number.
- [x] Post-commit best-effort: lifecycle notification + `PLAN_ACTIVATED` integration event.
- [x] BDM-direct `activatePlan` route delegates to the helper.
- [x] Branch `approvalHandlers.sales_goal_plan` on action AND on doc_type: reject → `buildGroupBReject`; approve | post + (PLAN_ACTIVATE | SALES_GOAL_PLAN) → call helper; approve | post + plan-adjacent doc_types (BULK_TARGETS_IMPORT / PLAN_NEW_VERSION / TARGET_REVISION) → close request with "must trigger {docType} action separately" reason.
- [x] Healthcheck `node backend/scripts/healthcheckSalesGoalPlanHubApprove.js` — **32/32 PASS**
- [x] `node -c` on universalApprovalController + salesGoalController — both green
- [x] `npx vite build` — green in 24.23s
- [ ] Live UI ratify deferred to bundled smoke at the end of PC3-PC7 sweep.

### Files touched

- modified: backend/erp/controllers/universalApprovalController.js — `sales_goal_plan` handler branches on action + doc_type
- new: backend/scripts/healthcheckSalesGoalPlanHubApprove.js
- modified: CLAUDE-ERP.md
- modified: docs/PHASETASK-ERP.md
...
> Same fix template as PC1-PC5, applied to `ic_transfer`. Cross-entity surface — Hub handler must branch on `request.doc_type` because the `ic_transfer` Hub key covers TWO physical models (InterCompanyTransfer + IcSettlement). Both helpers were already extracted in prep work; PC6 wires the Hub handler to them.

### Symptom

President clicks **Approve** on any IC_TRANSFER or IC_SETTLEMENT in /erp/approvals → `POST /api/erp/approvals/universal-approve 500 (Internal Server Error)`. Same bug class as PC1-PC5.

### Tasks

- [x] Helper `approveSingleIcTransfer(transferId, userId)` extracted from `approveTransfer` route (prep) — InterCompanyTransfer DRAFT → APPROVED + STATUS_CHANGE audit. Idempotent on APPROVED + downstream states (SHIPPED/RECEIVED/POSTED). No period lock (pre-financial state).
- [x] Helper `postSingleIcSettlement(settlementId, userId)` extracted from `postSettlement` route (prep) — IcSettlement DRAFT → POSTED + emits IC_SETTLEMENT TransactionEvent. Idempotent on POSTED. Period-locks against creditor entity. Wraps event creation + status flip in withTransaction. Guards empty settled_transfers.
- [x] BDM-direct `approveTransfer` + `postSettlement` routes delegate to the helpers.
- [x] Branch `approvalHandlers.ic_transfer` on action AND on doc_type: reject → `buildGroupBReject` with both InterCompanyTransfer + IcSettlement modelByDocType mapping; approve | post + IC_SETTLEMENT → `postSingleIcSettlement`; approve | post + IC_TRANSFER (default) → `approveSingleIcTransfer`.
- [x] Healthcheck `node backend/scripts/healthcheckIcTransferHubApprove.js` — **34/34 PASS**
- [x] `node -c` on universalApprovalController + interCompanyController + icSettlementController — all green
- [x] `npx vite build` — green in 24.23s
- [ ] Live UI ratify deferred to bundled smoke at the end of PC3-PC7 sweep.

### Files touched

- modified: backend/erp/controllers/universalApprovalController.js — `ic_transfer` handler branches on action + doc_type
- new: backend/scripts/healthcheckIcTransferHubApprove.js
- modified: CLAUDE-ERP.md
- modified: docs/PHASETASK-ERP.md
...
> Seventh and FINAL Group B Hub handler wired. Closes the "approve a BANK_RECON in /erp/approvals → 500" regression. **CAUTION: BankStatement.status === 'FINALIZED' is immutable — finalize cannot be undone without manual DB intervention.**

### Symptom

President clicks **Approve** on any BANK_RECON in /erp/approvals → `POST /api/erp/approvals/universal-approve 500 (Internal Server Error)`. Same bug class as PC1-PC6.

### Tasks

- [x] Helper `finalizeRecon(statementId, userId)` already factored as a service-layer function in `services/bankReconService.js` — no extraction needed.
- [x] Hub handler peeks BankStatement.status BEFORE calling finalizeRecon — re-clicking Approve from Hub returns `{ already_finalized: true }` instead of throwing the service-layer "Already finalized" error.
- [x] Branch `approvalHandlers.banking` on action: reject → `buildGroupBReject` (terminal-state guard against FINALIZED); approve | post → deref ApprovalRequest → status peek → `finalizeRecon` (only when not already FINALIZED) → close request to APPROVED with history $push.
- [x] Healthcheck `node backend/scripts/healthcheckBankingHubApprove.js` — **24/24 PASS**
- [x] `node -c` on universalApprovalController — green
- [x] `npx vite build` — green in 24.23s
- [ ] Live UI ratify deferred to bundled smoke at the end of PC3-PC7 sweep.
...
- modified: backend/erp/controllers/universalApprovalController.js — `banking` handler branches on action with idempotent status peek
- new: backend/scripts/healthcheckBankingHubApprove.js
- modified: CLAUDE-ERP.md
- modified: docs/PHASETASK-ERP.md
...
After PC7, all seven Group B Hub handlers branch correctly on action and dispatch through extracted lifecycle helpers. **190/190** static assertions across PC1-PC7 healthchecks. SaaS spin-out audit gate cleared on this surface.

### Final consolidated verification (May 01 2026 — sweep close)

Ran a single combined verification sweep across all seven phases after they all landed on dev:

- All 7 G6.7 healthchecks **PASS** (190 + 54 = 244 static assertions including the new combined sweep `healthcheckG67Pc4Through7HubApprove.js` that covers PC4-PC7 in one file).
- 5 sibling regressions **PASS** (IncomeProxy 32/32, ComputePayrollProxy 29/29, PayslipProxyRoster 31/31, SalesDiscount 41/41, TeamActivity 22/22).
- `npx vite build` green in **58.69s**.
- Require-load smoke: all 8 modified modules load cleanly via `require()` from a fresh Node process — no circular-import / missing-export break at module load. All 7 lifecycle helpers (3 PC3 helpers + PC4 + PC5 + PC6×2 + PC7's existing finalizeRecon) confirmed callable.
- Live Playwright ratify (President at `/erp/approvals`, May 01 2026 ~01:13 UTC): page rendered with 7 pending items, 0 console errors, sidebar Approvals badge `7`, full PageGuide intact, "Authority Matrix Disabled" badge visible. Screenshot: `g67-pc4-7-approvals-hub-7items-as-president.png`.
- Why no live approve-click smoke per-module: zero PC3-PC7 module items are pending in the dev cluster; seeding fresh fixtures for each module is genuinely heavy and out of scope for the close-out smoke. The static healthcheck wiring contract (244 assertions) is dispositive for the "approve throws → 500" regression class. The first real cycle the user runs through any of these surfaces will exercise the live approve path and ratify naturally — same posture PC3 took.

---

## Phase G9.R11 — Inbox Importance Triage (Apr 30 2026 evening)

> Closes the ">1000 [GUARD] inbox rows" complaint trail (started with the alerter retirement earlier this evening) by separating high-signal `briefing` from operational-noise digests in the inbox folder taxonomy and adding a priority filter inside `AI_AGENT_REPORTS`.

### Why
Two flaws in the existing folder routing surfaced when the user complained about inbox noise:
1. `briefing` (morning brief, FP&A forecast, procurement scorecard, expansion readiness) shared `AI_AGENT_REPORTS` with `compliance_alert` / `kpiVariance` / `ai_alert` / `ai_coaching` — high-signal daily reads buried under operational noise.
2. Four agents (`fefoAuditAgent`, `proxySlaAgent` × 2, `dataQualityAgent`) emitted categories not in `CATEGORY_TO_FOLDER` at all → those rows fell through to the main `INBOX` virtual folder. Even with `INBOX_HIDDEN_FOLDERS_BY_ROLE` hiding `AI_AGENT_REPORTS`, these orphan-category rows would still spill into the main count.

### Shipped
| File | Change |
|---|---|
| `backend/erp/utils/inboxLookups.js` | `FOLDER_DEFAULTS`: +1 row `EXECUTIVE_BRIEF` (sort_order 5). Existing AI_AGENT_REPORTS shifted 5→6, ANNOUNCEMENTS 6→7, etc. `CATEGORY_TO_FOLDER`: `briefing` → EXECUTIVE_BRIEF (was AI_AGENT_REPORTS); 4 orphan categories explicitly mapped to AI_AGENT_REPORTS. `HIDDEN_FOLDERS_BY_ROLE_DEFAULTS.president` adds `'AI_AGENT_REPORTS'` to `metadata.hidden_folders`. |
| `backend/scripts/backfillMessageInboxEntityId.js` | `CATEGORY_TO_FOLDER` copy synced (the "MUST stay in sync" comment is now enforced by healthcheck). |
| `backend/controllers/messageInboxController.js` | `ZERO_COUNTS` (+executive_brief), aggregation pipeline (+executive_brief sum), new `?priority=` query param threaded into the filter (single value or comma-separated). |
| `backend/scripts/migrateExecutiveBriefFolder.js` (NEW) | Idempotent, dry-run-by-default. Per entity: insert `EXECUTIVE_BRIEF` lookup row if missing, bump existing AI_AGENT_REPORTS sort_order 5→6 if still default, update or insert president `INBOX_HIDDEN_FOLDERS_BY_ROLE` row, re-point existing `MessageInbox` rows where `category=='briefing' AND folder=='AI_AGENT_REPORTS'` → `folder='EXECUTIVE_BRIEF'`. |
| `frontend/src/components/common/inbox/InboxFolderNav.jsx` | `ICON_BY_CODE.EXECUTIVE_BRIEF = Newspaper` (lucide-react). |
| `frontend/src/pages/common/InboxPage.jsx` | `DEFAULT_FOLDERS` (+1 row, sort_order shifted), `priorityFilter` state, chip row rendered conditionally when `activeFolder === 'AI_AGENT_REPORTS'`, `params.priority` threaded through `messageService.list`. CSS for chip row added inline. |
| `frontend/src/erp/components/WorkflowGuide.jsx` `'inbox'` + `frontend/src/components/common/PageGuide.jsx` `'inbox'` | Banner steps describe Executive Brief, priority chips, and the new president hidden-folder default (AI_AGENT_REPORTS in addition to APPROVALS). |
| `backend/scripts/healthcheckInboxImportanceTriage.js` (NEW) | 23-point static wiring contract — lookup defaults shape, category mapping symmetry between inboxLookups + backfill script, controller ZERO_COUNTS + aggregation, frontend icon + DEFAULT_FOLDERS + chip wiring, banner copy. |

### Verification (Apr 30 2026 evening)
- Healthcheck `node backend/scripts/healthcheckInboxImportanceTriage.js`: TBD (will run post-write).
- Vite build + `node -c` on changed backend files: TBD (will run post-write).
- Playwright smoke: login as president, see EXECUTIVE_BRIEF folder in rail (newspaper icon), confirm AI Agents folder count excluded from main INBOX count (open at `/inbox`), click into AI Agents tab, verify priority chip row renders with All / High / Important / Normal / Low chips, select "High" — list refreshes filtered.

### Subscriber tunability (Rule #3)
| To do | Path |
|---|---|
| Re-show AI Agents to president | Control Center → Lookup Tables → INBOX_HIDDEN_FOLDERS_BY_ROLE → edit `president` row, drop `'AI_AGENT_REPORTS'` from `metadata.hidden_folders` |
| Hide AI Agents for CEO | Same lookup, add `{ code: 'ceo', metadata: { hidden_folders: ['APPROVALS','AI_AGENT_REPORTS'] } }` |
| Hide TASKS for finance | Same lookup, add `{ code: 'finance', metadata: { hidden_folders: ['TASKS'] } }` |
| Add a new priority tier | Free-text on schema; UI chip row needs a code change. Lift to `MESSAGE_PRIORITY` lookup if subscribers ask. |
| Re-route a category to a different folder | Code-resident today (CATEGORY_TO_FOLDER). Lift to `MESSAGE_CATEGORY_FOLDER_MAP` lookup with `insert_only_metadata: true` if subscribers ask. |
...
4. (Optional) `node backend/scripts/tempInboxArchiveGuards.js --apply --soft-delete` to clear historical `[GUARD]` pile per-recipient.
